...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
        ...
        public function __construct() {
            ...
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'WPsmartcrm' ),
                'menu_title'                     => __( 'Install Plugins', 'WPsmartcrm' ),
                'installing'                     => __( 'Installing Plugin: %s', 'WPsmartcrm' ),
                'oops'                           => __( 'Something went wrong.', 'WPsmartcrm' ),
                ...
                'return'                         => __( 'Return to Required Plugins Installer', 'WPsmartcrm' ),
                'dashboard'                      => __( 'Return to the dashboard', 'WPsmartcrm' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'WPsmartcrm' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'WPsmartcrm' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'WPsmartcrm' ),
                'dismiss'                        => __( 'Dismiss this notice', 'WPsmartcrm' ),
            );
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'WPsmartcrm' ) . '">' . __( 'Return to the Dashboard', 'WPsmartcrm' ) . '</a>' ) . '</p>';
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'WPsmartcrm' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'WPsmartcrm' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'WPsmartcrm' ) . '</a>'  : '';
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'WPsmartcrm', 'WPsmartcrm', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'WPsmartcrm', 'WPsmartcrm', $rendered, $nag_class );
                }
            }

            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'WPsmartcrm' );
            }
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'WPsmartcrm' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'WPsmartcrm' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'WPsmartcrm' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'WPsmartcrm' ) : __( 'Recommended', 'WPsmartcrm' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'WPsmartcrm' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'WPsmartcrm' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'WPsmartcrm' ) . ' %2$s">' . __( 'Install', 'WPsmartcrm' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'WPsmartcrm' ) . ' %2$s">' . __( 'Activate', 'WPsmartcrm' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'WPsmartcrm' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'WPsmartcrm' ),
                'source' => __( 'Source', 'WPsmartcrm' ),
                'type'   => __( 'Type', 'WPsmartcrm' ),
                'status' => __( 'Status', 'WPsmartcrm' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => __( 'Install', 'WPsmartcrm' ),
                'tgmpa-bulk-activate' => __( 'Activate', 'WPsmartcrm' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'WPsmartcrm' ), $imploded );
                }
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', 'WPsmartcrm' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'WPsmartcrm' );
                $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'WPsmartcrm' );
                $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'WPsmartcrm' );
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'WPsmartcrm' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'WPsmartcrm' );
            ...
            public function activate_strings() {
                ...
                $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'WPsmartcrm' );
                $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'WPsmartcrm' );
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'WPsmartcrm' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'WPsmartcrm' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'WPsmartcrm' ) . '</span><span class="hidden">' . __( 'Hide Details', 'WPsmartcrm' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'WPsmartcrm' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'WPsmartcrm' );
                }
                // Default installation strings.
                else {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'WPsmartcrm' );
                    $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'WPsmartcrm' );
                    $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'WPsmartcrm' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'WPsmartcrm' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'WPsmartcrm' ) . '</span><span class="hidden">' . __( 'Hide Details', 'WPsmartcrm' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'WPsmartcrm' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'WPsmartcrm' );
                }
            ...
            public function bulk_footer() {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'WPsmartcrm' ) . '">' . __( 'Return to the Dashboard', 'WPsmartcrm' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
