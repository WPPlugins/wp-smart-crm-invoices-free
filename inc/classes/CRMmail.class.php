...
class testCRM_mail{
	...
	public function __construct($params)
	{
		...
		elseif(is_array($params) ){
			...
			isset($params['ID_agenda']) ? $this->ID_agenda = $params['ID_agenda'] : $this->ID_agenda =$ID_agenda;
			isset($params['ID_doc']) ? $this->ID_doc = $params['ID_doc'] : $this->ID_doc = $ID_doc;
			isset($params['ID_docRow']) ? $this->ID_docRow = $params['ID_docRow'] : $this->ID_docRow =$ID_docRow;
			isset($params['sendNow']) ? $this->sendNow = $params['sendNow'] : $this->sendNow =$sendNow;
	 ...
	 *
	 **/
	public function get_userMail($userid){
	...
	public function setCustomerMailSubject( $date, $time, $context, $row, $customSubject=""){
		...
		switch($context){
			case "appointment":
//				$subject= "Appuntamento con personale ".$business." in data: $date alle ore $time";
				$subject= sprintf( __( 'Appointment with staff %1$s on %2$s', 'WPsmartcrm' ), $business, $date);
				$subject.="\r\n";
				break;
			case "expiring service":
//				$subject="$business: Servizio in scadenza in data: $date\r\n";
				$subject= sprintf( __( '%1$s: Exipiring service on %2$s', 'WPsmartcrm' ), $business, $date);
				$subject.="\r\n";
				break;
			case "purchase":
			//	$subject="$business: Acquisto in data: $date\r\n";
				$subject= sprintf( __( '%1$s: purchase on %2$s', 'WPsmartcrm' ), $business, $date);
				$subject.="\r\n";
				break;
			case "generic":
	...
	public function setCustomerMailBody($date, $time, $context, $row, $customBody=""){
		...
		switch($context){
			case "appointment":
				...
				$body= sprintf( __( 'Dear %1$s, we remind you the appointment with our staff on %2$s at %3$s', 'WPsmartcrm' ), $name, $date, $time );
				$body.="\r\n";
				$body.=__( 'Kind regards', 'WPsmartcrm' );
				$body.=PHP_EOL;
				break;
			case "expiring service":
//				$body="Gentile $name,\r\n";
//				$body.="$business vi ricorda  che il servizio : ".$mailRow['rowDescription'].$art." &egrave; in scadenza in data: $date\r\n";
				$body= sprintf( __( 'Dear %1$s, %2$s reminds you that the service: %3$s will expire on %4$s', 'WPsmartcrm' ), $name, $business, $mailRow['rowDescription'], $date );
				break;
			case "purchase":
				//$body="$business: Acquisto in data: $date\r\n";
				$body= sprintf( __( '%1$s: purchase on %2$s', 'WPsmartcrm' ), $business, $date);
				$body.="\r\n";
				break;
			case "generic":
				$body= $customBody;
	...
	public function setUserMailSubject($date, $time, $context, $row){
		...
		switch($context){
			case "appointment":
				$subject= sprintf( __( 'Appointment on %1$s with %2$s', 'WPsmartcrm' ), $date, $name );
				break;
			case "todo":
				$subject= sprintf( __( 'Todo on %s', 'WPsmartcrm' ), $date);
				break;
			case "expiring service":
				$subject= sprintf( __( 'Expiring service: %1$s on %2$s', 'WPsmartcrm' ), $mailRow['rowDescription'], $date );
				break;
			case "expired payment":
				$subject= sprintf( __( 'Payment expired on %1$s invoice # %2$s', 'WPsmartcrm' ), $date, $id_fattura);
				break;
			case "purchase":
				$subject= sprintf( __( 'Purchase on %1$s by %2$s', 'WPsmartcrm' ), $date, $name );
				break;
		}
		return stripslashes($subject);
	}
	...
	public function setUserMailBody( $date,$time, $context, $row, $subject, $annotations){
		...
		switch($context){
			case "appointment":
//				$body="Ti ricordiamo l'appuntamento con  $name in data: $date alle ore $time\r\n";
				$body= sprintf( __( 'We remind you the appointment with %1$s on %2$s at %3$s', 'WPsmartcrm' ), $name, $date, $time );
				break;
			case "todo":
//				$body="Ti ricordiamo l'attività $subject in data: $date\r\n";
				$body= sprintf( __( 'We remind you the activity %1$s on %2$s', 'WPsmartcrm' ), $subject, $date);
			    break;
			case "expiring service":
//				$body="Ti ricordiamo che il servizio : ".$mailRow['rowDescription'].$art.", sottoscritto da $name scadr&agrave; in data: $date\r\n";
				$body= sprintf( __( 'We remind you that the service: %1$s by %2$s will expire on %3$s', 'WPsmartcrm' ), $mailRow['rowDescription'].$art, $name, $date );
				break;
			case "expired payment":
//				$body="Ti ricordiamo che il pagamento della fattura # $id_fattura di importo € $amount da $name &egrave; scaduto in data: $date\r\n";
				$body= sprintf( __( 'We remind you that the payment of invoice # %1$s amount %2$d expired on %3$s', 'WPsmartcrm' ), $id_fattura, $amount, $date );
				break;
			case "purchase":
//				$body=" Acquisto in data: $date\r\n";
				$body= sprintf( __( 'Purchase on %1$s', 'WPsmartcrm' ), $date );
				break;
		}
		return $body;
	}
	 ...
	 * @param int $ID_docRow
	 */
	public function sendNotificationNow($e_from,$e_to,$e_subject,$e_body, $e_date, $ID_agenda,$ID_doc,$ID_docRow){
		global $wpdb;
		...
		if ($e_to)
		{
		    ...
		    if ( wp_mail( $e_to , $e_subject , $e_body , $headers ) )
		    {
		        ...
		        $wpdb->prepare(
			        "INSERT INTO $table
			        ( e_from, e_to, e_subject, e_body,e_date,fk_agenda,fk_documenti,fk_documenti_dettaglio, e_sent)
...
class CRM_mail{
						...
						$this->setQueue(
							...
							if ($this->sendNow==1)
                            {
    							$this->sendMailNow(
								    ...
								    $this->get_userMail($mailtoUser),
								    $this->setUserMailSubject($this->CRM_email_date($activityDate,0, $date_format)->pretty_date , $activityTime,  $activity['context'], $activity['rowID']),
								    $this->setUserMailBody($this->CRM_email_date($activityDate,0, $date_format)->pretty_date, $activityTime, $activity['context'], $activity['rowID'], $activity['subject']),
								    $this->CRM_email_date($activityDate,$step->ruleStep, $date_format)->mysql_date,
								    $activity['id'],
								    $activity['documentID'],
								    $activity['rowID']
	 ...
	 *sets subject for email to customers
	 **/
	public function setCustomerMailSubject( $date, $time, $context, $row){
		$name=$this->oCustomer->get_business_name();
		$mailRow=$this->oDocument->get_documentRow($row);
		$options=get_option('CRM_business_settings');
		$business=$options['business_name'];
...
//				$subject="$business: Servizio in scadenza in data: $date\r\n";
	 ...
	 *sets body for email to customers
	 **/
	public function setCustomerMailBody($date, $time, $context, $row){
		$name=$this->oCustomer->get_business_name();
		$mailRow=$this->oDocument->get_documentRow($row);
		$options=get_option('CRM_business_settings');
		$business=$options['business_name'];
