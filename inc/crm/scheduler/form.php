...
switch ($tipo_agenda)
{
    case 1:
        ...
        $icon='<i class="glyphicon glyphicon-tag"></i> '.__('New TODO','WPsmartcrm');
        break;
    case 2:
        $icon='<i class="glyphicon glyphicon-pushpin"></i> '.__('New Appointment','WPsmartcrm');
        break;
    case 3:
        $icon='<i class="glyphicon glyphicon-option-horizontal"></i> '.__('New Activity','WPsmartcrm');
        break;
    default:
        $tipo="";
        break;
...
<script type="text/javascript">
	jQuery(document).ready(function ($) {
		...
		$('#fk_clienti').kendoDropDownList({
			optionLabel : "<?php _e('Select Customer','WPsmartcrm') ?>...",
			dataSource: _clients,
		...
		var validator = $("form").kendoValidator({
			...
			messages: {
				hasDays:"<?php _e('You should select how many days in advance to activate the notification','WPsmartcrm')?>",
				hasNoty:"<?php _e('You should select a user or a group of users to notify to','WPsmartcrm')?>",
				hasClients: "<?php _e('You should select a customer','WPsmartcrm')."."; $tipo_agenda==1 ? "<br /> "._e('To send an internal communication select you company','WPsmartcrm') :null ?>",
				hasObject: "<?php _e('You should type a subject for this Eevent','WPsmartcrm')?>",
				//hasUsers: "<?php _e('You should select at least one user','WPsmartcrm')?>",
			}
		}).data("kendoValidator");
		...
		$("form").validate({
			submitHandler: function () {
				showMouseLoader();
				$('#btn_save b').html("<?php _e('Saving...','WPsmartcrm')?>");
    ...
    <!-- TAB 1 -->
        ...
        <div id="d_anagrafica">
            ...
            <div class="row form-group">
                <label class="col-sm-2 control-label"><?php _e('Customer','WPsmartcrm')?> *</label>
            ...
            <?php if ($tipo_agenda==2) {?>
            <div class="row form-group">
                <label class="col-sm-4 control-label"><?php _e('Start', 'WPsmartcrm' ); ?>
                    <input name="data_scadenza_inizio" id='data_scadenza_inizio'  value="<?php echo $data_scadenza?>" class="" required validationMessage="<?php _e('You should select a start date/time for this appointment','WPsmartcrm')?>">
                </label> 
                <label class="col-sm-4 control-label"><?php _e('End', 'WPsmartcrm' ); ?>
                    <input name="data_scadenza_fine" id='data_scadenza_fine'  value="<?php echo $data_scadenza?>" class="" required validationMessage="<?php _e('You should select an end date/time for this appointment','WPsmartcrm')?>">
                </label>
            </div>
            <?php } ?>
            <?php if ($tipo_agenda==1) { ?>
            <div class="row form-group">
                <label class="col-sm-2 control-label"><?php _e('TODO Date','WPsmartcrm')?> *</label>
                <div class="col-sm-4">
                    <input type="text" name="data_scadenza_inizio" id='data_scadenza_inizio' value="<?php echo $data_scadenza?>"  required validationMessage="<?php _e('You should select an expiration date for this event','WPsmartcrm')?>">
                </div>
            </div>
            <?php } ?>
        <h4 class="page-header" style="background:#e2e2e2;padding:15px"><?php _e('Contents','WPsmartcrm')?></h4>
            <div class="row form-group">
	            <label class="col-sm-2 control-label"><?php _e('Subject','WPsmartcrm')?> *</label>
	            <div class="col-sm-4">
                    <input type="text" value="<?php if(isset($oggetto)) echo $oggetto?>" name="oggetto" id="oggetto" class="form-control  k-textbox _m" placeholder="<?php _e('Type a subject for this Event','WPsmartcrm')?>" >
	            </div>
	        
            </div>
            <div class="row form-group">
                <label class="col-sm-2 control-label"><?php _e('Description','WPsmartcrm')?></label>
	            <div class="col-sm-4">
            ...
            <div class="row form-group">
                <label class="col-sm-2 control-label"><?php _e('Priority','WPsmartcrm')?></label>
                <div class="col-sm-4">
                <?php if(isset($riga["priorita"])) WPsCRM_priorita($riga["priorita"]); else WPsCRM_priorita()?> 
                </div>
            </div>

            <h4 class="page-header" style="background:#e2e2e2;padding:15px"><?php _e('Notification rules','WPsmartcrm')?><span class="crmHelp" data-help="notification-rules"></span></h4>


            <div class="row form group" style="padding-bottom:20px;border-bottom:1px solid #ccc">
               <label class="col-sm-2 control-label"><?php _e('Days in advance','WPsmartcrm')?> *</label>
                <div class="col-sm-4">
                    <select class="form-control ruleActions _m k-dropdown _flat" style="width:150px" id="ruleStep" name="ruleStep">
                        <option value=""><?php _e("Sel","WPsmartcrm")?></option><?php for($k=0;$k<31;$k++){echo '<option value="'.$k.'">'.$k.'</option>'; } ?>

                    </select>
                </div>
                <label class="col-sm-2 control-label"><?php _e('Send also instant notification','WPsmartcrm')?></label>
                <div class="col-sm-4">
            ...
            <div class="row form-group" style="border:1px solid red;line-height: 3.2em;<?php echo $tipo_agenda==1 ? "display:none" : false ?>">
                <label class="col-sm-2 control-label" style="font-size:1.2em"><?php _e('Send email to customer','WPsmartcrm')?></label>
                <div class="col-md-4">
                <input type="checkbox" class="ruleActions " id="remindToCustomer" name="remindToCustomer"/> 
                </div>
            </div>
            <div class="row for-group">
              <label class="col-sm-2 control-label"  style="line-height:20px"><?php _e('Send mail to recipients','WPsmartcrm')?></label>
                <div class="col-md-4">
                <input type="checkbox" class="ruleActions " id="mailToRecipients" name="mailToRecipients"/>
                </div>
            </div>
            <div class="row form-group" style="margin-top:10px">
                <label class="col-sm-2 control-label" style="line-height:20px"><?php if($tipo_agenda==2) _e('Select Account for this appointment','WPsmartcrm'); else _e('Send to User','WPsmartcrm')?></label>
                <div class="col-md-4">
                    <input class="ruleActions" id="remindToUser" name="remindToUser" />
                </div>

                 <label class="col-sm-2 control-label" style="line-height:20px"><?php if($tipo_agenda==2) _e('Publish on Account dashboard','WPsmartcrm'); else _e('Publish on User dashboard','WPsmartcrm')?>?</label> 
                 <div class="col-md-4">
            ...
            <div class="row form-group"  <?php if($tipo_agenda==2) echo ' style="display:none"'?> >
                <label class="col-sm-2 control-label"><?php _e('Send to Group','WPsmartcrm')?></label>
                <div class="col-md-4">
                    <input class="ruleActions" id="remindToGroup" name="remindToGroup">
                </div>
                    <label class="col-sm-2 control-label"><?php _e('Publish on dashboard','WPsmartcrm')?>?</label>
                <div class="col-md-4">
             ...
             <div class="row form-group">
                 <ul class="select-action" style="margin-left:8px">
                    <li class="btn btn-success btn-sm _flat" id="btn_save"><i class="glyphicon glyphicon-floppy-disk"></i> 
                        <b onClick="return false;"> <?php _e('Save','WPsmartcrm')?></b>
                    </li>
                    <li class="btn btn-warning btn-sm _flat"><i class="glyphicon glyphicon-floppy-remove"></i>
                        <b onClick="window.location.replace('<?php echo admin_url('admin.php?page=smart-crm&p=scheduler/list.php')?>');return false;"> <?php _e('Reset','WPsmartcrm')?></b>
                    </li>
