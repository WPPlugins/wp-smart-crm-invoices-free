...
<script type="text/javascript">
	...
	function elimina()
	{
		if (!confirm("<?php _e('Confirm delete? It will still be possible to recover the deleted Customer ','WPsmartcrm')?>"))
			return;
		...
		function save(e) {
    		...
    		var customerValidator = jQuery("#form_insert").kendoValidator({
    			...
    			messages: {
					hasCountry: "<?php _e('You should select customer country','WPsmartcrm')?>",
    				hasFiscalCode: "<?php _e('You should type customer VALID Tax Code or Vat Number','WPsmartcrm')?>",
    				hasName: "<?php _e('You should type customer First Name or Business Name.','WPsmartcrm')?>",
    				hasLastName: "<?php _e('You should type customer Last Name or Business Name.','WPsmartcrm')?>",
    				hasBusinessName: "<?php _e('You should type customer Business name or First Name and Last Name','WPsmartcrm')?>",
    				hasVAT: "<?php _e('You should type customer VAT CODE if you use the \'Business name\' field','WPsmartcrm')?>",
    				isUser:"<?php _e('To create a new WP user you must set Email, Username and Password','WPsmartcrm')?>",
    				isFiscallyDefined:"<?php _e('Please check private or business','WPsmartcrm')?>"
    			}
    		}).data("kendoValidator");
				...
				jQuery.ajax({
					...
					success: function (response) {
						...
						if (response.indexOf('OK') != -1) {
							...
							hideMouseLoader();
							noty({
								text: "<?php _e('Customer has been saved','WPsmartcrm')?>",
								layout: 'center',
								type: 'success',
							...
							noty({
							text: "<?php _e('Something was wrong','WPsmartcrm')?>" + ": " + response,
							layout: 'center',
							type: 'error',
							template: '<div class="noty_message"><span class="noty_text"></span></div>',
							closeWith: ['button'],
...
<input type="hidden" name="ID" id="ID" value="<?php echo $ID?>">
    ...
    <h3><?php if ($ID) { ?> <?php _e('Customer','WPsmartcrm')?>: <?php echo "<span class=\"header_customer\">".stripslashes($cliente)."</span>";
			  } else{
        ?> <?php _e('New Customer','WPsmartcrm')?> <?php } ?>
    </h3>
    ...
    <ul class="select-action">
        ...
        <li class="btn btn-warning btn-sm _flat">
            ...
            <b onClick="annulla();return false;"> <?php _e('Reset','WPsmartcrm')?></b>
        </li>

        <li class="btn btn-danger btn-sm _flat" style="margin-right:10px">
            <i class="glyphicon glyphicon-remove"></i>
            <b onClick="elimina();return false;"> <?php _e('Delete','WPsmartcrm')?></b>
        </li>
    ...
    <div id="tabstrip" style="margin-top:14px">
        <ul>
            <li id="tab1"><?php _e('Master Data','WPsmartcrm')?></li>
            <?php
			if ($ID){
            ?>
            <li id="tab2"><?php _e('Contacts','WPsmartcrm')?></li>
            <li id="tab3"><?php _e('Notes','WPsmartcrm')?></li>
            <li id="tab4"><?php _e('Summary','WPsmartcrm')?></li>
            <?php 
        ...
        <div>
            <div id="d_anagrafica">
				...
				<div class="row form-group">
					<label class="col-sm-1 control-label">
						<?php _e('Invoiceable?','WPsmartcrm')?>
					</label>
					<div class="col-sm-4">
                        <span style="margin-right:20px"><input type="radio" name="fatturabile" id="fatturabile_1" value="1" <?php if (isset($riga) && $riga["fatturabile"]==1) echo "checked"?> /><?php _e('Yes','WPsmartcrm')?></span>
                        <span><input type="radio" name="fatturabile" id="fatturabile_2" value="0" <?php if (isset($riga) && $riga["fatturabile"]==0) echo "checked"?> /><?php _e('No','WPsmartcrm')?></span>
					</div>

					<label class="col-sm-1 control-label">
						<?php _e('Type','WPsmartcrm')?>
					</label>
					<div class="col-sm-4">
						<input type="radio" name="tipo_cliente" value="1" <?php if (isset($riga) && $riga["tipo_cliente"]==1) echo "checked"?> /><?php _e('Private','WPsmartcrm')?>
						<input type="radio" name="tipo_cliente" value="2" <?php if (isset($riga) && $riga["tipo_cliente"]==2) echo "checked"?> /><?php _e('Business','WPsmartcrm')?>
					</div>
				</div>
                <div class="row form-group">
                    <label class="col-sm-1 control-label"><?php _e('Country','WPsmartcrm')?></label>
                    <div class="col-sm-4">
                    ...
                    <label class="col-sm-1 control-label"><?php _e('Business Name','WPsmartcrm')?></label>
                    <div class="col-sm-4">
                ...
                <div class="row form-group">
                    <label class="col-sm-1 control-label"><?php _e('Tax code','WPsmartcrm')?></label>
                    <div class="col-sm-4">
                        <input type="text" name="cod_fis" id="cod_fis" value="<?php if(isset($riga)) echo $riga["cod_fis"]?>" class="form-control _toCheck"  readonly title="<?php _e('Select country first','WPsmartcrm') ?>...">
                    </div>
                    <label class="col-sm-1 control-label"><?php _e('VAT number','WPsmartcrm')?></label>
                    <div class="col-sm-4">
                        <input type="text" name="p_iva" id="p_iva" value="<?php if(isset($riga)) echo $riga["p_iva"]?>" class="form-control _toCheck"  readonly title="<?php _e('Select country first','WPsmartcrm') ?>...">
                    </div>
                </div>
                <div class="row form-group">
                    <label class="col-sm-1 control-label"><?php _e('First Name','WPsmartcrm')?></label>
                    <div class="col-sm-4">
                        <input type="text" name="nome" value="<?php if(isset($riga)) echo stripslashes($riga["nome"])?>" class="form-control">
                    </div>
                    <label class="col-sm-1 control-label"><?php _e('Last Name','WPsmartcrm')?></label>
                    <div class="col-sm-4">
                ...
                <div class="row form-group">
                    <label class="col-sm-1 control-label"><?php _e('Place of birth','WPsmartcrm')?></label>
                    <div class="col-sm-4">
                ...
                <div class="row form-group">
                    <label class="col-sm-1 control-label"><?php _e('Date of birth','WPsmartcrm')?></label>
                    <div class="col-sm-4">
                        <input type="text" id="data_nascita" name="data_nascita" value="<?php if(isset($riga)) echo WPsCRM_inverti_data($riga["data_nascita"])?>">
                    </div>
                    <label class="col-sm-1 control-label"><?php _e('Category','WPsmartcrm')?></label>
                    <div class="col-sm-4">
                    ...
                    <script>
                    	...
                    	jQuery(document).ready(function ($) {
							$('#customerCategory').kendoMultiSelect({
								...
								dataValueField: "id",
								dataSource: cats,
								placeholder: "<?php _e('Select','WPsmartcrm')?>",
								noDataTemplate: '<?php _e('No Categories; create categories in CRM settings ->Customers settings page','WPsmartcrm')?>',
								change: function () {
					...
					<script>
                    	...
                    	jQuery(document).ready(function ($) {
                    		$('#customerInterests').kendoMultiSelect({
								...
								placeholder: "<?php _e('Select','WPsmartcrm')?>",
								dataSource: ints,
								noDataTemplate: '<?php _e('No Interests; create interests in CRM settings ->Customers settings page','WPsmartcrm')?>',
								change: function () {
                ...
                <div class="row form-group">
                    <label class="col-sm-1 control-label" <?php echo $style_disabled?>><?php _e('Agent','WPsmartcrm')?>:</label>
                    ...
                    <label class="col-sm-1 control-label"><?php _e('Comes from','WPsmartcrm')?>:</label>
					<div class="col-sm-4">
					...
					<script>
                    	...
                    	jQuery(document).ready(function ($) {
							$('#customerComesfrom').kendoMultiSelect({
								...
								placeholder: "<?php _e('Select','WPsmartcrm')?>",
								dataSource: provs,
								noDataTemplate: "<?php _e('No Origins; create origins in CRM settings ->Customers settings page','WPsmartcrm')?>",
								change: function () {
                ...
                ?>
                <!--<div class="row form-group">
                    <label class="col-sm-1 control-label"><?php _e('Create WP user','WPsmartcrm')?>?</label>
                    <div class="col-sm-1">
                        <input type="checkbox" name="crea_utente" value="1">
                    </div>
                </div>
                <div class="row form-group">
                    <label class="col-sm-1 control-label"><?php _e('Username','WPsmartcrm')?></label>
                    <div class="col-sm-4">
                        <input type="text" name="username" size="20" maxlength='50' value="" class="form-control">
                    </div>
                    <label class="col-sm-1 control-label"><?php _e('Password','WPsmartcrm')?></label>
                    <div class="col-sm-4">
        ...
        <div>
            <!--<h2 style="text-align:center"><?php _e('Notes','WPsmartcrm')?></h2>-->
            <div style="min-height: 200px">
                <div id="annotation">
                    <h3 style="text-align:center"><?php _e('Notes Timeline','WPsmartcrm')?></h3>
                    <div>
...
<script>
	...
	var invoiceWindow = $("#createInvoice").kendoWindow({
		width: "90%",
		height: "84%",
		title: "<i class=\"glyphicon glyphicon-fire\"></i> <?php _e('Create invoice for','WPsmartcrm');if(isset($cliente)) echo " ". stripslashes($cliente)?>",
		...
		width: "90%",
		height: "84%",
		title: "<i class=\"glyphicon glyphicon-send\"></i> <?php _e('Create quote for','WPsmartcrm');if(isset($cliente)) echo " ". stripslashes($cliente)?>",
    	...
    	$('#cd-timeline').on('click','.glyphicon-remove', function () {
    		...
    		$.ajax({
    			...
    			success: function (response) {
    				console.log(response);
    				noty({
    					text: "<?php _e('Annotation has been deleted','WPsmartcrm')?>",
    					layout: 'center',
    					type: 'success',
    					template: '<div class="noty_message"><span class="noty_text"></span></div>',
    					//closeWith: ['button'],
...
//update delle activity da modale
    	...
    	$('#nazione').kendoDropDownList({
    		placeholder: "<?php _e('Select country','WPsmartcrm') ?>...",
    	});
    	var country = jQuery("#nazione").data("kendoDropDownList").value();
    	if (country != "0") {
    		...
    		$('._toCheck').attr({ 'readonly': 'readonly' , 'title': '<?php _e('Select country first','WPsmartcrm') ?>...' , 'alt': '<?php _e('Select country first','WPsmartcrm') ?>...' })
    	}
