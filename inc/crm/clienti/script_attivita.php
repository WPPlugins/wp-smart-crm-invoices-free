...
jQuery(document).ready(function ($) {
	...
	$("#dialog_attivita").kendoWindow({
		width: "900px",
		height: "80%",
		title: "<?php _e('Add activity for Customer:','WPsmartcrm') ?>",
		visible: false,
		modal: true,
		draggable: false,
		resizable:false,
		...
		close: function () {
			this.title("<?php _e('Add activity for Customer:','WPsmartcrm') ?>");
			$('.modal_loader').hide();
	...
	var an_validator = $("#new_activity").kendoValidator({
		...
		messages: {
			hasAnnotation: "<?php _e('You should type some text','WPsmartcrm')?>",
		}
	}).data("kendoValidator");
	...
	$("#saveActivity").click(function () {
		...
		activityTimestamp = $("#data_attivita").data('kendoDatePicker').value()
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (opener == "clienti") {//ricarico la grid solo se aperto da form clienti
						...
						noty({
							text: "<?php _e('Annotation has been added','WPsmartcrm')?>",
							layout: 'center',
							type: 'success',
