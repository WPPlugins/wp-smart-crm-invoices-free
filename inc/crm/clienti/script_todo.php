...
<script>
	jQuery(document).ready(function ($) {
		$("#dialog_todo").kendoWindow({
			width: "86%",
			height: "80%",
			title: "<?php _e('Add todo for Customer:','WPsmartcrm') ?>",
			visible: false,
			modal: true,
			draggable: false,
			resizable:false,
			...
			close: function () {
				...
				this.title("<?php _e('Add todo for Customer:','WPsmartcrm') ?>");
				$('.modal_loader').hide();
			}
		})
			...
			$.ajax({
				...
				success: function (response) {
					if (opener == "clienti") {//ricarico la grid solo se aperto da form clienti
						...
						noty({
							text: "<?php _e('TODO has been added','WPsmartcrm')?>",
							layout: 'center',
							type: 'success',
		...
		var t_validator = $("#new_todo").kendoValidator({
			...
			messages: {
				...
				hasClients: "<?php _e('You should select a client','WPsmartcrm')?>",
				hasObject: "<?php _e('You should type a subject for this item','WPsmartcrm')?>",
				hasExpiration:"<?php _e('You should select  date for this event','WPsmartcrm')?>"
