...
<form name="form_insert" action="<?php echo admin_url('admin.php?page=smart-crm&p=documenti/insert.php&type=1&ID='.$ID.'&security='.$update_nonce); echo isset($_REQUEST['layout']) ? "&layout=".$_REQUEST['layout'] : null?>" method="post" id="form_insert">
        ...
        <h1 style="text-align:center"><?php _e('CREATE/EDIT QUOTE','WPsmartcrm')?> <i class="glyphicon glyphicon-send"></i></h1>
<div id="tabstrip">
    <ul>
        <li id="tab1"><?php _e('DOCUMENT','WPsmartcrm')?></li>
        <!--<li><?php _e('BODY','WPsmartcrm')?></li>-->
        <?php 

        //if($generalOptions['services']==1){?>
        <!--<li><?php _e('SERVICES/PRODUCTS','WPsmartcrm')?></li>-->
        <?php // } ?>
        <li  id="tab2" onclick="aggiornatot();"><?php _e('COMMENTS AND INTERNAL DATA','WPsmartcrm')?></li>
    </ul>
    <!--PRIMO TAB -->
    <div>
        
        <h4 class="page-header" style="margin: 10px 0 20px;"><?php _e('DOCUMENT DATA','WPsmartcrm')?><span class="crmHelp" data-help="document-data"></span>
	        <span style="float:right;margin-top: -7px;">
                <label class="col-sm-2 control-label"><?php _e('Number','WPsmartcrm')?></label><span class="col-sm-2"><input name="progressivo" class="form-control" data-placement="bottom" title="<?php _e('Number','WPsmartcrm')?>" value="<?php if (isset($riga)) echo $riga["progressivo"]?>" readonly disabled/>
	        </span></span></h4>

        <div class="row form-group">
	        <label class="col-sm-1 control-label"><?php _e('Date','WPsmartcrm')?></label>
	        <div class="col-sm-2"><input name="data" id="data" class="_m" data-placement="bottom" title="<?php _e('Date','WPsmartcrm')?>" value="<?php echo $data?>" style="border:none"/>
	        </div>
            <!--<div class="col-sm-2 hide_sm"></div>-->
            <label class="col-sm-2 control-label" style="color:firebrick"><?php _e('Expiration date','WPsmartcrm')?></label>
            ...
            <label class="control-label" style="margin-left:20px"><?php _e('Accepted','WPsmartcrm')?>?</label>
            <div class="col-sm-1">
                <input type="checkbox" name="pagato" value="1" <?php if (isset($riga)) echo $riga["pagato"]?"checked":""?>>
            </div>
        </div>
        <div class="row form-group">
	        <label class="col-sm-1 control-label"><?php _e('Subject','WPsmartcrm')?></label>
	        <div class="col-sm-2"><input type="text" class="form-control col-md-10" name="oggetto" id="oggetto"  value="<?php if (isset($riga)) echo $oggetto?>">
                ...
                <!--<div class="col-sm-2 hide_sm"></div>-->
            <label class="col-sm-2 control-label"><?php _e('Reference','WPsmartcrm')?></label>
        ...
        <div class="row form-group">
            <label class="col-sm-2 control-label"><?php _e('Notes','WPsmartcrm')?></label><br />
    ...
    <h4 class="page-header"><?php _e('CUSTOMER DATA','WPsmartcrm')?><span class="crmHelp" data-help="customer-data"></span>
             <?php
    if (isset($fk_clienti))
    {
...
<?php }?>        
	    ...
	    <h4 class="page-header"><?php _e('QUOTE TEXT','WPsmartcrm')?> <span class="crmHelp" data-help="quotation-text"></span></h4>
            <div class="inside">
            <?php
        ...
        <h4 class="page-header"><?php _e('Add Products to quote','WPsmartcrm')?><span class="crmHelp" data-help="quotation-products"></span>
            <?php do_action("WPsCRM_show_WOO_products");?>
		</h4>
        <div class="row form-group">
            <div class="col-md-6">
                <span  id="btn_manual" class="btn btn-sm  btn-add_desc_row _flat" onclick="add_manual_row();" style="margin-left:0px"><?php _e('Add row with price','WPsmartcrm')?> &raquo;</span>
                <span id="btn_descriptive" class="btn btn-sm  btn-add_desc_row _flat" onclick="add_descriptive_row();"><?php _e('Add descriptive row','WPsmartcrm')?> &raquo;</span>
            </div>
        </div>
        <div class="row form-group">
            ...
            <thead>
            <tr>
	            <th><?php _e('Code','WPsmartcrm')?></th>
	            <th><?php _e('Description','WPsmartcrm')?></th>
	            <th><?php _e('Q.ty','WPsmartcrm')?></th>
	            <th><?php _e('Price','WPsmartcrm')?></th>
	            <th><?php _e('Discount','WPsmartcrm')?></th>
	            <th><?php _e('VAT','WPsmartcrm')?></th>
	            <th><?php _e('Total','WPsmartcrm')?></th>
	            <th><?php _e('Actions','WPsmartcrm')?></th>
            </tr>
            </thead>
            <tbody>
        ...
        if ($ID)
        {
            ...
            ?>
 		            <tr class="riga" id="r_<?php echo $i?>">
			            ...
			            <td><button  onclick="elimina_riga(<?php echo $rigad["id"]?>, <?php echo $i?>);return false;"><?php _e('Delete','WPsmartcrm')?></button></td>
		            </tr>
            <?php		}
                else
                {
        ...
        <div class="row form-group">
	        <label class="col-sm-1 control-label"><?php _e('Total Net','WPsmartcrm')?></label>
	        ...
	        <label class="col-sm-1 control-label"><?php _e('Total Tax','WPsmartcrm')?></label>
	        ...
	        <label class="col-sm-1 control-label"><?php _e('Total','WPsmartcrm')?></label>
    ...
    <div>
        <div class="row form-group">
            <label class="col-sm-2 control-label"><?php _e('Quote value (required)','WPsmartcrm')?> *</label>
             <div class="col-sm-6">
        ...
        <div class="row form-group">
	        <label class="col-sm-2 control-label"><?php _e('Forecast success percentage','WPsmartcrm')?> % </label>
	        <div class="col-sm-4">
...
//modal validators
    ...
    var validator = jQuery("#form_insert").kendoValidator({
    	...
    	messages: {
    		...
    		hasValue: "<?php _e('This quote should have a value','WPsmartcrm')?>",
    	}
    }).data("kendoValidator");
...
function addRow(id, codice, descrizione, iva, prezzo, arr_rules, rule)
	{
		...
		jQuery('#t_art').append('<tr class="riga" id="r_' + n_row + '"><td><input type="hidden" name="tipo_' + n_row + '" value="1"><input type="hidden" name="id_' + n_row + '" value="' + id + '"><input type="text" size="10" name="codice_' + n_row + '" id="codice_' + n_row + '" value="' + codice + '"></td><td><textarea  name="descrizione_' + n_row + '" id="descrizione_' + n_row + '" style="width:93%">' + descrizione + '</textarea></td><td><input type="text" size="4" name="qta_' + n_row + '" id="qta_' + n_row + '"  onblur="aggiorna(' + n_row + ')" value="1" /></td><td><input type="text" size="10" name="prezzo_' + n_row + '" id="prezzo_' + n_row + '" value="' + prezzo + '"  onblur="aggiorna(' + n_row + ')"></td><td><input type="text" size="4" name="sconto_' + n_row + '" id="sconto_' + n_row + '" size="5"  onblur="aggiorna(' + n_row + ')"></td><td><input type="text" size="4" name="iva_' + n_row + '" id="iva_' + n_row + '" value="' + iva + '" size="5"  onblur="aggiorna(' + n_row + ')"></td><td><input type="text" size="4" name="totale_' + n_row + '" id="totale_' + n_row + '"></td><td><button type="button"  onclick="elimina_riga(0,' + n_row + ')"><?php _e('Delete','WPsmartcrm')?></button></td></tr>');
		aggiorna(n_row);
	}
    ...
    function add_manual_row() {
        jQuery.ajax({
            ...
            success: function (result) {
                ...
                var n_row=jQuery('#t_art > tbody > tr').length+1;
                jQuery('#t_art').append('<tr class="riga" id="r_' + n_row + '"><td><input type="hidden" name="tipo_' + n_row + '" value="2"><input type="text" size="10" name="codice_' + n_row + '" id="codice_' + n_row + '" value=""></td><td><textarea rows="1" style="width:93%"  name="descrizione_' + n_row + '" id="descrizione_' + n_row + '" class="descriptive_row"></textarea></td><td><input type="text" size="4" name="qta_' + n_row + '" id="qta_' + n_row + '" onblur="aggiorna(' + n_row + ')"></td><td><input type="text" size="10" name="prezzo_' + n_row + '" id="prezzo_' + n_row + '" value=""  onblur="aggiorna(' + n_row + ')"  oninput="aggiorna(' + n_row + ')"></td><td><input type="text" size="4" name="sconto_' + n_row + '" id="sconto_' + n_row + '" size="5"  onblur="aggiorna(' + n_row + ')"  oninput="aggiorna(' + n_row + ')"></td><td><input type="text" size="4" name="iva_' + n_row + '" id="iva_' + n_row + '" value="' + iva + '" size="5"  onblur="aggiorna(' + n_row + ')" oninput="aggiorna(' + n_row + ')"></td><td><input type="text" size="4" name="totale_' + n_row + '" id="totale_' + n_row + '"></td><td><button type="button"  onclick="elimina_riga(0,' + n_row + ')"><?php _e('Delete','WPsmartcrm')?></button></td></tr>');
            },
            error: function (errorThrown) {
                console.log(errorThrown);
            }
    ...
    function add_descriptive_row() {
        var n_row = jQuery('#t_art > tbody > tr').length + 1;
        jQuery('#t_art').append('<tr class="riga" id="r_' + n_row + '"><td colspan="7"><input type="hidden" name="tipo_' + n_row + '" value="3"><textarea  rows="1" style="width:93%" name="descrizione_' + n_row + '" id="descrizione_' + n_row + '"class="descriptive_row" ></textarea></td><td><button type="button"  onclick="elimina_riga(0,' + n_row + ')"><?php _e('Delete','WPsmartcrm')?></button></td></tr>');
    }
    function annulla() {
    ...
    function elimina_riga(id_art, riga) {
        if (!confirm("<?php _e('Confirm delete? It will still be possible to recover the deleted item ','WPsmartcrm')?>"))
            return false;
