...
<style>
	...
	.k-tabstrip .k-content.k-state-active {background-color:#ddd!important}
...
<form name="form_insert" action="<?php echo admin_url('admin.php?page=smart-crm&p=documenti/insert.php&type=3&ID='.$ID.'&security='.$update_nonce); echo isset($_REQUEST['layout']) ? "&layout=".$_REQUEST['layout'] : null?>" method="post" id="form_insert">
    ...
    <h1 style="text-align:center;background-color:#ffd7d7"><?php _e('CREATE/EDIT INFORMAL INVOICE','WPsmartcrm')?> <i class="glyphicon glyphicon-list-alt"></i></h1>
    <div id="tabstrip">
        <ul>
            <li id="tab1"><?php _e('INVOICE','WPsmartcrm')?></li>
            <li onclick="aggiornatot();"><?php _e('COMMENTS AND INTERNAL DATA','WPsmartcrm')?></li>
        </ul>
        <div>
            <h4 class="page-header" style="margin: 10px 0 20px;">
                <?php _e('INVOICE DATA','WPsmartcrm')?><span class="crmHelp" data-help="document-data"></span>
                <?php if ($ID) {?>
                <span style="float:right;font-size:.8em;text-decoration:underline;cursor:pointer" class="_edit_header"><i class="glyphicon glyphicon-pencil"></i> <?php _e('Edit','WPsmartcrm')?></span>
                <?php } ?>
                <span style="float:right;margin-top: -7px;">
                    <label class="col-sm-2 control-label"><?php _e('Number','WPsmartcrm')?></label>
                    <span class="col-sm-2">
                        <input name="progressivo" id="progressivo" class="form-control" data-placement="bottom" title="<?php _e('Number','WPsmartcrm')?>" value="<?php echo $riga["progressivo"]?>" readonly disabled />
                    </span>
                </span>
            <span style="float:right;margin-top: -7px;" class="col-md-4">
                <label class="control-label"><?php _e('Issue Date','WPsmartcrm')?></label>
				<?php if ($ID) {?>
	            <span class="col-sm-4" style="margin-top: -4px;">
					<input name="data" id="data" class="form-control  _m" data-placement="bottom" title="<?php _e('Date','WPsmartcrm')?>" value="<?php echo  $data ?>" style="border:none" />
	            </span>
				<?php } else {?>
                <span class="col-sm-4" style="margin-top: -4px;">
                    <input name="data" id="data" class="form-control _m" data-placement="bottom" title="<?php _e('Date','WPsmartcrm')?>" value="" style="border:none"  />
                    
                </span>
				<?php } ?>
			</span>
                <div class="row" id="edit_warning" style="display:none;font-size:.8em;color:red;margin-top:20px"><div class="col-md-4 pull-right"><?php _e('WARNING: edit date and number may cause incongruences in your accounting','WPsmartcrm')?></div></div>
            </h4>
            <div class="row form-group">
                <!--<label class="col-sm-1 control-label"><?php //_e('Object','WPsmartcrm')?></label>
                <div class="col-sm-3"><input type="text" class="form-control " name="oggetto" id="oggetto"  maxlength='50' value="<?php echo $oggetto?>">
                </div>-->
                <label class="col-sm-1 control-label"><?php _e('Reference','WPsmartcrm')?></label>
                <div class="col-sm-3">
                    <input type="text" class="form-control" name="riferimento" id="riferimento" maxlength='55' value="<?php echo $riga["riferimento"]?>">
                </div>
                <label class="col-sm-2 control  -label"><?php _e('Notes','WPsmartcrm')?></label>
                <div class="col-sm-4">
                    ...
                    <small><i>(<?php _e('Will be shown in the document','WPsmartcrm')?>)</i></small>
                </div>
            </div>
            <div class="row form-group">
                <hr />
                <label class="col-sm-1 control-label"><?php _e('Payment methods','WPsmartcrm')?></label>
                <div class="col-sm-2">

                    <select name="modalita_pagamento" id="modalita_pagamento" class="_form-control col-md-12">
                        <option value="0" <?php if($payment==0) echo "selected"?>><?php _e('Select','WPsmartcrm')?></option>
                        <?php
					foreach($arr_payments as $pay)
					{
						$pay_label=explode('~',$pay);
						if( !empty($pay_label[1])) $_pay_label=$pay_label[0]." (".$pay_label[1]." ".__('dd','WPsmartcrm').")";
                ...
                <label class="control-label"><?php _e('Payment exp. Date','WPsmartcrm')?></label>
                <div class="col-sm-2">
                ...
                {
                ?>
                <label class="control-label" style="margin-left:20px"><?php _e('Paid','WPsmartcrm')?>?</label>
                <div class="col-sm-1">
                ...
                ?>
                <label class="control-label" style="margin-left:20px"><?php _e('Notify','WPsmartcrm')?>? </label>
                <div class="col-sm-1">
            ...
            <section id="notifications" style="display:none!important">
                <h4 class="page-header"><?php _e('Invoice payment reminder','WPsmartcrm')?> </h4>

                <div class="row form-group">
                    <label class="col-sm-1"><?php _e('Send to User','WPsmartcrm')?></label><div class="col-sm-2"><input class="ruleActions" id="remindToUser" name="remindToUser" /></div>
                    <label class="col-sm-1"><?php _e('Send to Group','WPsmartcrm')?></label><div class="col-sm-2"><input class="ruleActions" id="remindToGroup" name="remindToGroup"></div>
                    <label class="col-sm-1"><?php _e('Days after expiration','WPsmartcrm')?></label><div class="col-sm-2"><input class="ruleActions" id="notificationDays" name="notificationDays" type="number" value="<?php echo $giorni_pagamento?>"><small id="changeNoty"><a href="#" onclick="return false;"><?php _e('Edit default value','WPsmartcrm')?>&raquo;</a></small></div>
            ...
            <h4 class="page-header">
                <?php _e('CUSTOMER DATA','WPsmartcrm')?><span class="crmHelp" data-help="customer-data"></span>
                <?php
			if ($fk_clienti)
			{
                ...
                <ul class="select-action _forminvoice" style="float:right;/*transform:scale(.8);*/background-color:transparent;margin-top:-10px;width:inherit">
                    ...
                    <li class="btn _edit _white btn-sm _flat">
                        ...
                        <b> <?php _e('EDIT CUSTOMER DETAILS','WPsmartcrm')?></b>
                    </li>
                    <li style="display:none" class="btn btn-danger _quitEdit btn-sm _flat">
                        <i class="glyphicon glyphicon-close"></i>
                        <b> <?php _e('QUIT EDITING','WPsmartcrm')?></b>
                    </li>
                    <li class="btn"><i class="_tooltip glyphicon glyphicon-menu-right"></i></li>
                    <li class="btn btn-info btn-sm _flat btn_todo" style="margin-left:10px" title="<?php _e('NEW TODO','WPsmartcrm')?>">
                        <i class="glyphicon glyphicon-tag"></i>
                        <b> </b>
                    </li>
                    <li class="btn  btn-sm _flat btn_appuntamento" title="<?php _e('NEW APPOINTMENT','WPsmartcrm')?>">
                        <i class="glyphicon glyphicon-pushpin"></i>
                        <b> </b>
                    </li>
                    <li class="btn btn-primary btn-sm _flat btn_activity" title="<?php _e('NEW ANNOTATION','WPsmartcrm')?>">
            ...
            <div class="customer_data_partial" data-customer="<?php echo $fk_clienti?>">
                ...
                <div class="row form-group">
                    <label class="col-sm-1 control-label"><?php _e('Customer','WPsmartcrm')?></label>
                    <div class="col-sm-3">
                        <?php
						if ($fk_clienti)
						{
                    ...
                    <!--<label class="col-sm-1 control-label"><?php _e('Contact','WPsmartcrm')?></label>
                    <div class="col-sm-3">
                        <input type="hidden" id="FK_contatti" name="FK_contatti" value="<?php echo $FK_contatti?>" data-value="<?php echo $FK_contatti?>" />
                        
                    </div>-->
                    <div class="col-sm-2">
                        <input type="button" class="btn btn-sm btn-success _flat" id="save_client_data" name="save_client_data" value="<? _e('Save','WPsmartcrm')?>" style="display:none" />
                    </div>
                </div>
                <div class="row form-group">
                    <label class="col-sm-1 control-label"><?php _e('Address','WPsmartcrm')?></label>
                    <div class="col-sm-3">
                    ...
                    <label class="col-sm-1 control-label"><?php _e('ZIP code','WPsmartcrm')?></label>
                    <div class="col-sm-2">
                    ...
                    <label class="col-sm-1 control-label"><?php _e('C.F.','WPsmartcrm')?></label>
                    <div class="col-md-2">
                ...
                <div class="row form-group">
                    <label class="col-sm-1 control-label"><?php _e('Town','WPsmartcrm')?></label>
                    <div class="col-sm-3">
                    ...
                    <label class="col-sm-1 control-label"><?php _e('State/Prov.','WPsmartcrm')?></label>
                    <div class="col-sm-2">
                    ...
                    <label class="col-sm-1 control-label"><?php _e('VAT code','WPsmartcrm')?></label>
                    <div class="col-md-2">
            ...
            <h4 class="page-header"><?php _e('Add Products to invoice','WPsmartcrm')?><span class="crmHelp" data-help="invoice-products"></span>
        ...
        <div>
            <div class="row form-group">
                <label class="col-sm-2 control-label"><?php _e('Comments','WPsmartcrm')?></label>
                <div class="col-sm-6">
	...
	if ($ID)
	{
        ...
        <li class="btn btn-sm btn-info _flat" onclick="location.replace('?page=smart-crm&p=documenti/document_print.php&id_invoice=<?php echo $ID?>')">
            ...
            <b> <?php _e('Printable version','WPsmartcrm')?></b>
        </li>
        <?php
	}
        ?>
...
<div id="dialog-view" style="display:none;margin: 0 auto; text-align: center; z-index: 1000; width: 100%; height: 100%; background: url('<?php echo str_replace("inc/crm/documenti/","",plugin_dir_url( __FILE__ ))?>css/img/bg_w_tr.png');position: absolute;left: 0;top:0;"  class="_modal" data-from="documenti">
    ...
    <div class="panel-heading" style="padding: 3px 10px;">
        <h3 style="text-align:center;margin-top: 8px;"><?php _e('Change default days','WPsmartcrm')?><span class="crmHelp" data-help="deafult-invoice-payment-noty"></span></h3>
    </div>
    <div class="panel-body" style="padding:50px">
        <label><?php  _e('Change default value','WPsmartcrm')?></label><input class="ruleActions" name="new_default_noty" id="new_default_noty" type="number" value="<?php echo $documentOptions['invoice_noty_days']?>">
        <span class="btn btn-success btn-sm _flat" id="notyConfirm"><?php _e('Confirm','WPsmartcrm')?></span>
        <span class="btn btn-warning btn-sm _flat _reset" ><?php _e('Reset','WPsmartcrm')?></span>
    </div>
    </div>
</div>
...
<div id="reverseCalculator">
	<div class="col-md-11">
		<label><?php _e('Input full amount for reverse calculation:','WPsmartcrm') ?></label><input class="form-control" type="number" id="reverseAmount" />
	</div>
    <div class="col-md-11">
        <label><?php _e('Input refund for reverse calculation:','WPsmartcrm') ?></label><input class="form-control" type="number" id="reverseRefund" />
    </div>
    <div class="col-md-11"><br />
        <input class="btn _flat btn-success" type="button" id="calculate" value="<?php _e('Calculate:','WPsmartcrm') ?>" />
    </div>
</div>
<span id="informalNotification" style="display:none;width:100%!important"></span>
<script id="informalTemplate" type="text/x-kendo-template">
...
<script type="text/javascript">
	jQuery(document).ready(function ($) {
		...
		width: "400px",
		height: "300px",
		title: "<?php _e('Calculate from full amount:','WPsmartcrm') ?>",
		visible: false,
		modal: true,
		draggable: false,
		resizable:false,
	...
	<ul>\n\
		<li class=\"no-link\">\n\
			...
			<span class=\"btn btn-warning _flat\"><i class=\"glyphicon glyphicon-envelope\"></i> = <?php _e('NEW MAIL','WPsmartcrm')?></span>\n\
		</li>\n\
	</ul>"
    });
		var mainValidator = jQuery("#form_insert").kendoValidator({
			...
			messages: {
				//hasExpiration: "<?php _e('You should select an expiration date; select today for a non-relevant ','WPsmartcrm')?>",
				hasNoty: "<?php _e('You should select a user or a group of users to notify to','WPsmartcrm')?>",
				hasClient: "<?php _e('You should select a customer','WPsmartcrm')?>",
				hasRows: "<?php _e('You should add at least one row to this invoice','WPsmartcrm')?>",
				hasDescription:"<?php _e('Description is mandatory','WPsmartcrm')?>"
			}
		}).data("kendoValidator");
	...
	//usato solo per i controlli sulle contabilità avanzate che richiedono il tipo cliente per le ritenute d'acconto
	...
	<?php if ($ID){ ?>
		...
		$('._edit').on('click', function () {
			...
			$('#save_client_data').parent().append("<br><small class=\"_notice notice notice-error \"><?php _e("You're editing the master data for this customer",'WPsmartcrm')?></small>")
		...
		$('#save_client_data').on('click', function () {
			...
			$.ajax({
				...
				success: function (result) {
					console.log(result);
					noty({
						text: "<?php _e('Data has been saved','WPsmartcrm')?>",
						layout: 'center',
						type: 'success',
  ...
  var clienti=$('#fk_clienti').kendoDropDownList({
      placeholder: "<?php _e('Select Customer','WPsmartcrm')?>...",
...
/*
	    ...
	    $.ajax({
	        ...
	        success: function (result) {
	            ...
	            if (isNaN(result) == false) {
	                ...
	                noty({
	                    text: "<?php _e('Option Saved','WPsmartcrm')?>",
	                    layout: 'center',
	                    type: 'success',
	                ...
	                noty({
	                    text: "<?php _e('An error occurred','WPsmartcrm')?>",
	                    layout: 'center',
	                    type: 'error',
	                    template: '<div class="noty_message"><span class="noty_text"></span><span class="noty_close glyphicons gypicons-close"></span></div>',
	                    closeWith: ['button'],
	...
	var _informal = $("#informalNotification").kendoNotification({
		button: false,
		autoHideAfter: 0,
		position: {
			pinned: true,
	...
	_informal.show("<?php _e("WARNING: you're working on an INFORMAL INVOICE","WPsmartcrm")?>", "error");
