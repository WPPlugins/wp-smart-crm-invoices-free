...
<div id="schedulerWindow" style="text-align:center"><button onclick='onCopy()' class="btn btn-sm _flat"><i class="glyphicon glyphicon-copy"></i> <?php _e('Copy Event','WPsmartcrm')?></button> <?php _e('OR','WPsmartcrm')?> <button class="btn btn-sm _flat" onclick='onEdit()'><i class="glyphicon glyphicon-pencil"></i> <?php _e('Edit Event','WPsmartcrm')?></button></div>
<style>
...
<script>
	...
	function createScheduler(type) {
		...
		scheduler.addEvent({
			title: "<?php _e('New','WPsmartcrm')?> " + type ,
			tipo_agenda: typeID,
			esito:"",
...
jQuery(document).ready(function ($) {
	...
	$("#kscheduler").kendoScheduler({
		...
		editable: {
			...
			confirmation: "<?php _e('Are you sure you want to delete this event?','WPsmartcrm') ?>"
		},
		selectable:true,
		dataBound: function (e) {
			$('.k-more-events span').text('<?php _e('More events','WPsmartcrm') ?>...');
		...
		edit: function (e) {
			...
			popupEditor.setOptions({
				title: '<?php _e('Edit','WPsmartcrm') ?> ' + e.event.title,
				width: 940,
				height: 700
			})
...
<script id="event-day-template" type="text/x-kendo-template">
    ...
    <div class="activity-template activity-day-template  #if(des_tipo_agenda=='TODO'){# todo-event #}else{# appointment-event #}#">
        ...
        <div class="col-md-6">
            <span><strong><?php _e('Start','WPsmartcrm') ?>:</strong> #=kendo.toString(kendo.parseDate(start), $formatDateTime)#</span>
            <span><strong><?php _e('End','WPsmartcrm') ?>:</strong> #=kendo.toString(kendo.parseDate(end), $formatDateTime)#</span>
            <span class="event_description"><strong><?php _e('Description','WPsmartcrm') ?>:</strong> #=description#</span>
            <span><strong><?php _e('Status','WPsmartcrm') ?>:</strong> #=icon#</span>
        </div>
        <div class="col-md-4 pull-right">
            <span style="line-height:40px;height:44px;padding-bottom:3px;border-bottom:1px solid \#ccc;margin-bottom:3px;"><strong><?php _e('Customer','WPsmartcrm') ?>: </strong><a href="<?php  echo admin_url('admin.php?page=smart-crm&p=clienti%2Fform.php')?>&ID=#=id_cliente#" style="text-decoration:underline">#=cliente# <img src="#=img_cliente#" style="width:40px;height:40px;border-radius:50%;float:right;cursor:pointer;" /></a></span><br />
            <span style="line-height:40px;height:44px;padding-bottom:3px;"><strong><?php _e('Assigned to','WPsmartcrm') ?>:</strong> #=nome_agente# <img src="#=img_agente#" style="width:40px;height:40px;border-radius:50%;float:right" /></span>
        </div>
    </div>
</script>
<!--/DAILY VIEW EVENT TEMPLATE-->
...
<script id="event-agenda-template" type="text/x-kendo-template">
    ...
    <div class="activity-template activity-agenda-template  #=eventClass#">
        ...
        #if(status==2) iconStatus='<i class="glyphicon glyphicon-ok" style="color: green;font-size:1.2em" title="<?php _e('Done','WPsmartcrm') ?>"></i>'; if(status==1) iconStatus='<i class="glyphicon glyphicon-bookmark" style="color: black;font-size:1.2em" title="<?php _e('To be done','WPsmartcrm') ?>"></i>'#
        #esito != null ? iconStatus +="<small>(" + esito +")</small>" : null#
        <h4 style="text-align:center;text-transform:uppercase;margin-bottom:24px"><mark style="float:left;font-size:1.3em" class="mark-#=des_tipo_agenda#">#=iconEvent#</mark><span class="btn btn-sm btn-info edit-event" data-uid="#=uid#" style="float:left;display:inline-block;width:120px;margin-left:10px"><?php _e('Edit','WPsmartcrm') ?></span>#=title#</h4>
        <div class="col-md-6">
            <span><strong><?php _e('Start','WPsmartcrm') ?>:</strong> #=kendo.toString(kendo.parseDate(start), $formatDateTime)#</span>
            <span><strong><?php _e('End','WPsmartcrm') ?>:</strong> #=kendo.toString(kendo.parseDate(end), $formatDateTime)#</span>
            <span class="event_description"><strong><?php _e('Description','WPsmartcrm') ?>:</strong> #=description#</span>
            <span><strong><?php _e('Status','WPsmartcrm') ?>:</strong> #=iconStatus#</span>
            <span class="btn btn-sm btn-info quick-edit-event" data-id="#=id_agenda#" data-status="#=status#" data-esito="#=esito#" style="float:left;display:inline-block;width:120px;margin-left:10px"><?php _e('Quick Edit','WPsmartcrm') ?></span>
        </div>
        <div class="col-md-4 pull-right">
            <span style="line-height:40px;height:44px;padding-bottom:3px;border-bottom:1px solid \#ccc;margin-bottom:3px;"><strong><?php _e('Customer','WPsmartcrm') ?>: </strong><a href="<?php  echo admin_url('admin.php?page=smart-crm&p=clienti%2Fform.php')?>&ID=#=id_cliente#" style="text-decoration:underline">#=cliente# <img src="#=img_cliente#" style="width:40px;height:40px;border-radius:50%;float:right;cursor:pointer;" /></a></span><br />
            <span style="line-height:40px;height:44px;padding-bottom:3px;"><strong><?php _e('Assigned to','WPsmartcrm') ?>:</strong> #=nome_agente# <img src="#=img_agente#" style="width:40px;height:40px;border-radius:50%;float:right" /></span>
...
<script id="tooltip-template" type="text/x-kendo-template">
    <div class="scheduler_tooltip" style="text-align:left">
        ...
        #model.esito != null ? icon +="<small>(" + model.esito +")</small>" : null#
			...
			<span><strong><?php _e('Start','WPsmartcrm') ?>:</strong> #=kendo.toString(kendo.parseDate(model.start), $formatDateTime)#</span>
			<span><strong><?php _e('End','WPsmartcrm') ?>:</strong> #=kendo.toString(kendo.parseDate(model.end), $formatDateTime)#</span>
			<span class="event_description"><strong><?php _e('Description','WPsmartcrm') ?>:</strong> #=model.description#</span>
			<span><strong><?php _e('Status','WPsmartcrm') ?>:</strong> #=icon#</span>
			...
			<span style="line-height:40px;height:44px;padding-bottom:3px;"><strong><?php _e('Assigned to','WPsmartcrm') ?>:</strong> #=model.nome_agente# <img src="#=model.img_agente#" style="width:40px;height:40px;border-radius:50%;float:right" /></span>
		#} else {#
		<strong><?php _e('No event data is available','WPsmartcrm') ?></strong>
		#}#
</div>
</script>
<!-- /TOOLTIP EVENT TEMPLATE-->
