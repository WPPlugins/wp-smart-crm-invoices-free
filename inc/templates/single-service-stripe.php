...
get_header();
 ...
 if($subscription !="" && $subscription !=0){
     ...
     $length="<h5>".__('Subscription length','WPsmartcrm').": <strong>".$wpdb->get_row($SQL)->length." ". __('Months','WPsmartcrm')."</strong></h5>";
 ...
 $price_type=="month" ? $month= __("per month",'WPsmartcrm')." " : $month="" ;
 if($month !="")
 ...
 if(isset($_GET['customer']) && isset($_GET['invoice']) ){
     ...
     $_html.="<h3>". __('Thanks for your purchase','WPsmartcrm').", ". $wpdb->get_row($SQL)->nome." ". $wpdb->get_row($SQL)->cognome."</h3>";
     $_html.="<p>".__("We've sent you an email with your invoice.",'WPsmartcrm')."</p>";
     if(!is_user_logged_in()){
         $_html.="<p>".__("You can access your profile page after logging-in from the form here below. From there you can download your invoices and check status of your subscriptions.",'WPsmartcrm')."</p>";
         $_html.="<h5><a href=\"".get_post_permalink(get_the_ID())."\">".__("Return to the previous page.",'WPsmartcrm')."</a></h5>";
         $_html.="<h5><a href=\"".home_url()."\">".__("Return to homepage.",'WPsmartcrm')."</a></h5>";
         $_html.= "</pre>";
         echo $_html;
         //print_r($options);
     ...
     $_html.="<p>".__("You can access your profile page from the link here below. From there you can download your invoices and check status of your subscriptions.",'WPsmartcrm')."</p>";
         $_html.="<h5><a href=\"".get_page_link($user_page)."\">".__("Profile page.",'WPsmartcrm')."</a></h5>";
         $_html.="<h5><a href=\"".get_post_permalink(get_the_ID())."\">".__("Return to the previous page.",'WPsmartcrm')."</a></h5>";
         $_html.="<h5><a href=\"".home_url()."\">".__("Return to homepage.",'WPsmartcrm')."</a></h5>";
         $_html.= "</pre></div>";
         echo $_html;
...
?>
    <div class="_smartcrm_service container">
        ...
        <?php if ( has_post_thumbnail() ) 
            ...
            <form action="<?php echo get_permalink( $post->ID )?>" method="post"  id="stripeCheckout" >
                <div class="CRM_main_show_price">
                    <span><?php _e('PRICE','WPsmartcrm') ?>: <?php echo $showed_price." ".$month?><small>(+ iva)</small></span> <a class="button button-buy" data-amount="<?php echo $button_amount?>" data-code="<?php echo $code?>" data-subscription="<?php echo $subscription?>"><?php _e('Buy now','WPsmartcrm') ?> &raquo;</a>
                </div>

                    <div id="crmForm" style="display:none">
                        <ul>
                            <li><label><?php _e('First Name','WPsmartcrm')?></label><input type="text" id="CRM_firstname" name="CRM_firstname" value="<?php echo $usermeta['first_name'][0]?>" required /></li>
                            <li><label><?php _e('Last Name','WPsmartcrm')?></label><input type="text" id="CRM_lastname" name="CRM_lastname" value="<?php echo $usermeta['last_name'][0]?>" required/></li>
                            <li><label><?php _e('Email','WPsmartcrm')?></label><input type="email" id="CRM_email" name="CRM_email" value="<?php echo $email ?>" required /></li>
                            <li><label><?php _e('Confirm Email','WPsmartcrm')?></label><input type="email" id="CRM_cfmemail" name="CRM_cfmemail" value="<?php echo $email ?>" required /></li>
                            <li><label class="_address"><?php _e('Address','WPsmartcrm')?></label><input type="text" name="CRM_address" value="<?php echo $_address ?>" required /></li>
                            <li><label><?php _e('Town','WPsmartcrm')?></label><input type="text" name="CRM_town" value="<?php echo $_town ?>" required /></li>
                            <li><label><?php _e('ZIP Code','WPsmartcrm')?></label><input type="text" name="CRM_zip" value="<?php echo $_zip ?>"/></li>              
                            <li><label><?php _e('Phone','WPsmartcrm')?></label><input type="text" name="CRM_phone" value="<?php echo $_phone ?>" required /></li>
                            <li><label><?php _e('Client type','WPsmartcrm')?></label><?php _e('Private','WPsmartcrm')?>: <input type="radio" name="CRM_client_type" value="privato" <?php echo $checked1 ?>/> <?php _e('Business','WPsmartcrm')?>:<input type="radio" name="CRM_client_type" value="azienda" <?php echo $checked2 ?>/></li>
                            <li id="business_name" style="display:none"><label><?php _e('Business Name','WPsmartcrm')?></label><input type="text" id="CRM_businessname" name="CRM_businessname" value="<?php echo $_businessName ?>" /></li>
                            <li><label><?php _e('Cod. Fisc.','WPsmartcrm')?></label><input type="text" id="CRM_client_CF" name="CRM_client_CF" class="_fiscal" value="<?php echo $_cf ?>" required/></li>
                            <li><label><?php _e('Vat code','WPsmartcrm')?></label><input type="text" id="CRM_client_IVA" name="CRM_client_IVA" class="_fiscal" value="<?php echo $_piva ?>"/></li>
                            <hr />
                            <?php if($current_user->ID ==""){ ?>
                            <li><h5 ><span class="register_link"><?php _e('Register','WPsmartcrm')?> &raquo;</span> <small style="margin-left:30px;margin-right:20px;"><?php _e('Already registerd','WPsmartcrm')?>? <span class="show_login _highlighted"><?php _e('login here','WPsmartcrm')?>&raquo;</span></small></h5></li>
                            <li class="_register"><label><?php _e('Choose a username','WPsmartcrm')?></label><input name="CRM_username" id="CRM_username" type="text" required/></li>
                            <li class="_register"><label><?php _e('Choose a Password','WPsmartcrm')?></label><input type="password" name="CRM_password" id="CRM_password" /></li>
                            <li class="_register"><label><?php _e('Confirm Password','WPsmartcrm')?></label><input type="password" name="CRM_cfmpassword" id="CRM_cfmpassword" /></li>
                            ...
                            <?php  } ?>
                            <li id="sendToRegistration" style="display:none"><span id= "_register" onclick="_prepareToBuy()" class="button"><?php _e('Continue','WPsmartcrm')?>&raquo;</span></li>
                            <input type="submit" id="_send" value="<?php _e('Buy Now','WPsmartcrm')?>&raquo;" style="display:none" />
        ...
        <h3 style="margin-top:20px"><?php _e('Service Info and features','WPsmartcrm') ?>:</h3>
        <div class="CRM_service_features">
            <h5>
            <?php _e('Code','WPsmartcrm') ;?>: <strong><?php echo $code ?></strong>
            </h5>
            <?php if (count($terms) > 0) {?>
            <h5 class="services_category">
            
            <?php _e('CATEGORIES','WPsmartcrm') ?>: 
                <small>
                <?php 
            ...
            <?php } 
                ...
                if( $service_advice !=""){ ?>
                <h5>
                <?php _e('Additional info','WPsmartcrm') ;?>:<br /> <small><?php echo $service_advice ?></small>
                </h5>

                <?php }
                if(count($gallery_thumbs) > 0){ ?> 
                <h5 style="background:#fafafa" ><?php _e('Gallery','WPsmartcrm');?></h5>
...
<script>
    ...
    jQuery(document).ready(function ($) {
        ...
        if ($('input[name="CRM_client_type"]').filter(':checked').val() == "azienda") {
            $('#business_name').show();
            $('._address').html("<?php _e('Business Address','WPsmartcrm')?>");
            ...
            $('#business_name').hide();
            $('._address').html('<?php _e('Address','WPsmartcrm')?>');
