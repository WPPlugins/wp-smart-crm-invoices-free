...
$document = new WPsCRM_crmObject(array(
	...
	//"opening_text" => array("Con la presente, a seguito della Vostra gradita richiesta, Vi confermiamo la nostra migliore offerta per quanto segue", "Di seguito i dettagli della fattura"),
	//"closing_text" => array("Rimaniamo a Vostra disposizione per ogni chiarimento in merito alla presente offerta, e con l'occasione porgiamo distinti saluti", "Vi ringraziamo per il Vostro acquiesto, e con l'occasione porgiamo distinti saluti"),
	//"paid"=>"PAGATO"    
));
...
$document->master_data= function(){
    ...
    return array(
    array('full_header'=>"<h2 class=\"WPsCRM_businessName\">".html_entity_decode($options['business_name'])."</h2><br ><div class=\"WPsCRM_businessAddress\"> ".html_entity_decode($options['business_address'])." - ".html_entity_decode($options['business_zip']).", ".html_entity_decode($options['business_town'])."</div>",'show'=>1),
    array(__('Name','WPsmartcrm')=>html_entity_decode($options['business_name']),'show'=>0, 'show_label'=>0,''),
    array(__('Address','WPsmartcrm')=>html_entity_decode($options['business_address']),'show'=>0, 'show_label'=>0),
    array(__('Town','WPsmartcrm')=>html_entity_decode($options['business_town']),'show'=>0, 'show_label'=>0),
    array(__('Zip','WPsmartcrm')=>html_entity_decode($options['business_zip']),'show'=>0, 'show_label'=>0),
    array(__('Vat Code','WPsmartcrm')=>html_entity_decode($options['business_iva']),'show'=>1, 'show_label'=>1),
    array(__('Cod. Fisc','WPsmartcrm')=>html_entity_decode($options['business_cf']),'show'=>1, 'show_label'=>1),
    array(__('Phone','WPsmartcrm')=>html_entity_decode($options['business_phone']),'show'=>isset($options['show_phone']) ? $options['show_phone'] : 0, 'show_label'=>1),
    array(__('Fax','WPsmartcrm')=>html_entity_decode($options['business_fax']),'show'=>isset($options['show_fax']) ? $options['show_fax'] : 0, 'show_label'=>1),
    array(__('Email','WPsmartcrm')=>html_entity_decode($options['business_email']),'show'=>isset($options['show_email']) ? $options['show_email'] : 0 , 'show_label'=>1),
    array(__('Web Site','WPsmartcrm')=>html_entity_decode($options['business_web']),'show'=>isset($options['show_web']) ? $options['show_web'] : 0, 'show_label'=>1),
    array(__('IBAN','WPsmartcrm')=>html_entity_decode($options['business_iban']),'show'=>isset($options['show_iban']) ? $options['show_iban'] : 0, 'show_label'=>1),
	array(__('SWIFT','WPsmartcrm')=>html_entity_decode($options['business_swift']),'show'=>isset($options['show_swift']) ? $options['show_swift'] : 0, 'show_label'=>1)
    );
};
$document->numbering=function(){
    $options=get_option('CRM_documents_settings');
    return array(
        'invoices_prefix'=>$options['invoices_prefix'],
        'invoices_suffix'=>$options['invoices_suffix'],
        'invoices_start'=>$options['invoices_start'],
        'offers_prefix'=>$options['offers_prefix'],
        'offers_suffix'=>$options['offers_suffix'],
        'offers_start'=>$options['offers_start'],
        );
};
$document->messages=function(){
    $options=get_option('CRM_documents_settings');
    return array(
        'invoices_dear'=>$options['invoices_dear'],
        'invoices_before'=>$options['invoices_before'],
        'invoices_after'=>$options['invoices_after'],
        'offers_dear'=>$options['offers_dear'],
        'offers_before'=>$options['offers_before'], 
        'offers_after'=>$options['offers_after'],         
        );
};
$document->signature=function(){
     $options=get_option('CRM_documents_settings');
return '<div><img src="'.$options['crm_signature'].'" style="max-width:300px"></div>';
};
...
$document->formatted_signature=function(){
	$options=get_option('CRM_documents_settings');
return '<div class="formatted_signature" style="width:300px;display:inline-block;margin-top:20px;border-top:1px solid #ccc">'.$options['crm_formatted_signature'].'</div>';
};
...
function WPsCRM_get_clients_array($exclude_self=null) {
    ...
    $where="eliminato=0";
	if( $exclude_self ==null ) $where.=" AND ID_clienti <> 1"; 
...
function WPsCRM_get_clients2() {
    ...
    $where="eliminato=0";
	if( ! $client ) $where.=" AND ID_clienti <> 1";
    $current_user = wp_get_current_user();
    $user_id=$current_user->ID;
	if ( WPsCRM_is_agent() && ! WPsCRM_agent_can() )
	    $where.=" and agente=$user_id";
    if ($ragione_sociale=$_REQUEST["ragione_sociale"])
...
function WPsCRM_get_documents() {
	//current_user_can manage_options è admin
    ...
    foreach( $wpdb->get_results( $sql ) as $record){
        ...
        $pagato=$record->pagato ? __('Yes','WPsmartcrm'): "";
...
function WPsCRM_make_invoice() {
	if(check_ajax_referer('informal_document', 'security',false ) && current_user_can('manage_crm') ){
		...
		$wpdb->update(
			$d_table,
			array('progressivo' => "$new_reg", 'tipo' => '2'),
		  array('id'=>$doc_id),
			array('%s', '%d', '%d')
		  );
		  echo $wpdb->last_query;
...
function WPsCRM_get_scheduler() {
	...
	if( ! $client ) $where.=" AND fk_clienti <> 1"; 
    
    $sql="SELECT id_agenda, fk_clienti, fk_utenti_ins, oggetto, annotazioni, start_date, tipo_agenda, fk_subscriptionrules,esito, fatto FROM ".$a_table." where $where and eliminato=0 order by start_date asc";
        ...
        if ($record=$wpdb->get_row( $sql ))
        {
            ...
            foreach ($steps as $step)
            {
                ...
                if ( (in_array($user_id, $total_users) || ($user_role=='administrator' && $options['administrator_all']==1) || $user_id==$r_scheduler->fk_utenti_ins ) && $cond)
                {
                    ...
                    $arr_results[]=array("id_agenda"=>$r_scheduler->id_agenda, "cliente"=>stripslashes($cliente), "oggetto"=>stripslashes($r_scheduler->oggetto), "annotazioni"=>stripslashes($r_scheduler->annotazioni), "esito"=>stripslashes($r_scheduler->esito), "data_scadenza"=>$start_date, "status"=>$r_scheduler->fatto,"tipo_agenda"=>$tipo,"class"=>$rowClass." ".strtolower(str_replace(" ","_",$r_scheduler->fatto ) ), "destinatari"=>$destinatari." - ".$groups, "fk_utenti_ins"=>implode(',',array_unique(array_merge($adminArray,$activity_owner ) ) ) );
                }
    ...
    foreach( $wpdb->get_results( $sql ) as $r_scheduler)
    {
        ...
        if ($record=$wpdb->get_row( $sql ))
        {
            ...
            foreach ($steps as $step)
            {
				...
				if ( (in_array($user_id, $total_users) || ($user_role=='administrator' && $options['administrator_all']==1) || $user_id==$r_scheduler->fk_utenti_ins ) && $cond)
					{
						$arr_results[]=array("id_agenda"=>$r_scheduler->id_agenda, "tipo"=>$tipo, "oggetto"=>stripslashes($r_scheduler->oggetto), "annotazioni"=>stripslashes($r_scheduler->annotazioni), "esito"=>stripslashes($r_scheduler->esito), "data_scadenza"=>$r_scheduler->start_date , "status"=>$r_scheduler->fatto , "class"=>$rowClass." ".strtolower(str_replace(" ","_",$r_scheduler->fatto ) ),"destinatari"=>$destinatari." ".$groups, "fk_utenti_ins"=>implode(',',array_unique(array_merge($adminArray,$activity_owner ) ) ) );
...
//    else
//    {
        $CRMusers=get_users( array('role__in'=>array('CRM_agent', 'editor','administrator' ),'fields'=>array('ID','display_name' ) )  );
...
function WPsCRM_save_todo(){
	...
	if(check_ajax_referer('update_scheduler', 'security',false ) && current_user_can('manage_crm') ){
		...
		//diversificare per advanced
		$url = admin_url();
		$link="admin.php?page=smart-crm&event=".$fk_scheduler;
//		$annotazioni.=" (<a href='".$url.$link."'>".$url.$link."</a>)";
		$annotazioni.=" (<a href='".$url.$link."'><i class=\"glyphicon glyphicon-link\"></i></a>)";

		$_annotazioni=$timestamp_date."~".$user_firstname." ".$user_lastname. "~".__($n,'WPsmartcrm').": <b>".strtoupper($oggetto)."</b> ".$annotazioni."|";
		$result=$wpdb->query( 
			$wpdb->prepare(
			"UPDATE ".WPsCRM_TABLE."clienti SET annotazioni=concat(IFNULL(annotazioni,''),%s) WHERE ID_clienti = %d",
			$_annotazioni, $id_cliente
		   )
		);
		echo $wpdb->last_query;
	}
...
function WPsCRM_save_annotation(){
    ...
    $user_firstname=$current_user->user_firstname;
    $user_lastname=$current_user->user_lastname;
	$data_attivita= date( "Y-m-d" , strtotime( substr($_REQUEST['activityTimestamp'], 0, strpos($_REQUEST['activityTimestamp'], "(") ) ) );
    $annotazioni=$data_attivita."~".$user_firstname." ".$user_lastname. "~".$_POST['annotazioni']."|";
	$result=$wpdb->query( 
		$wpdb->prepare(
		"UPDATE $c_table SET annotazioni=concat(IFNULL(annotazioni,''),%s) WHERE ID_clienti=%d",
		$annotazioni, $id_cliente
	   )
		);
...
function WPsCRM_reload_annotation(){
    ...
    //$timeline=array_reverse(array_filter($timeline) );
	$index=0;
...
function WPsCRM_register_invoices() {
    if(check_ajax_referer('update_document', 'security',false ) && current_user_can('manage_crm') ){
        ...
        $result=$wpdb->query( 
        	 $wpdb->prepare(
        				"UPDATE $table SET registrato=1 WHERE tipo='2' and data>=%s and data<=%s",
        				$data_from, $data_to 
        			)
    	);
...
function WPsCRM_import_customers() 
{
	if(check_ajax_referer('update_document', 'security',false ) && current_user_can('manage_crm') ){
        ...
        if ($file)
        {
            ...
            {
                if (($riga["first_name"]&&$riga["last_name"]) || $riga["company"]) {
					...
					if (!$wpdb->insert(
					    $table,
					    array('FK_aziende' => "$ID_azienda",'nome' => addslashes($riga["first_name"]),'cognome' => addslashes($riga["last_name"]),'ragione_sociale' => addslashes($riga["company"]),'indirizzo' => addslashes($riga["address"]),'cap' => addslashes($riga["zip"]), 'localita' => addslashes($riga["city"]), 'provincia' => addslashes($riga["province"]), 'p_iva' => addslashes($riga["vat"]), 'cod_fis' => addslashes($riga["tax_code"]), 'telefono1' => addslashes($riga["phone1"]), 'telefono2' => addslashes($riga["mobile"]), 'fax' => addslashes($riga["fax"]), 'email' => addslashes($riga["email"]), 'sitoweb' => addslashes($riga["website"]), 'skype' => addslashes($riga["skype"]),'data_modifica'=>"$data_modifica",'data_inserimento'=>"$data_inserimento", 'data_nascita' => addslashes($riga["birth_date"]), 'luogo_nascita' => addslashes($riga["birth_place"]), 'nazione' => addslashes($riga["country"]), 'fatturabile' => $riga["invoiceable"], 'tipo_cliente' => $riga["type"], 'categoria' => $str_categories, 'provenienza' => $str_origins, 'interessi' => $str_interests),
					    array('%d','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%d','%d','%s','%s','%s')
					))
					{
					    //gestire errore mancato inserimento
					    $row_err=__( 'Generic error', 'WPsmartcrm' );
							...
							if ($rigan=$wpdb->get_row( $sql ))
							{
								$insert_in_mailinglist=$wpdb->update(
									$wpdb->prefix.'newsletter',
										array("list_1"=>$list1, "list_2"=>$list2, "list_3"=>$list3, "list_4"=>$list4, "list_5"=>$list5, "list_6"=>$list6, "list_7"=>$list7, "list_8"=>$list8, "list_9"=>$list9, "list_10"=>$list10, "list_11"=>$list11, "list_12"=>$list12, "list_13"=>$list13, "list_14"=>$list14, "list_15"=>$list15, "list_16"=>$list16, "list_17"=>$list17, "list_18"=>$list18, "list_19"=>$list19, "list_20"=>$list20),
										array('ID'=>$rigan->id),
										array('%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d')
								);
							}
							else
							{
								...
								$insert_in_mailinglist=$wpdb->insert(
									$wpdb->prefix.'newsletter',
									array('name' => $name, 'surname' => $surname,'sex' => 'n','status' => 'C','created' =>current_time('mysql', 1), 'email' =>  $riga["email"], 'token' => $token, "list_1"=>$list1, "list_2"=>$list2, "list_3"=>$list3, "list_4"=>$list4, "list_5"=>$list5, "list_6"=>$list6, "list_7"=>$list7, "list_8"=>$list8, "list_9"=>$list9, "list_10"=>$list10, "list_11"=>$list11, "list_12"=>$list12, "list_13"=>$list13, "list_14"=>$list14, "list_15"=>$list15, "list_16"=>$list16, "list_17"=>$list17, "list_18"=>$list18, "list_19"=>$list19, "list_20"=>$list20),
									array('%s','%s','%s','%s','%s','%s','%s','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d')
								);
							}	
						}						
					}
                ...
                else
                {
                    ...
                    $row_err="";
                    if (!$riga["first_name"])
                        $row_err.=sprintf( __( 'Missing field: %s', 'WPsmartcrm' ), "first_name")."; ";
                    if (!$riga["last_name"])
                        $row_err.=sprintf( __( 'Missing field: %s', 'WPsmartcrm' ), "last_name")."; ";
                    if (!$riga["company"])
                        $row_err.=sprintf( __( 'Missing field: %s', 'WPsmartcrm' ), "company");
                    $arr_err[]=array("riga"=>$row, "errore"=>$row_err);
                    $string_err.="#$row $row_err\n";
                }
                $row++;
        ...
        {   
            $msg_err=__( 'Could not read file. Try again.', 'WPsmartcrm' );
            header("Content-type: application/json");
        ...
        {
            $msg_ok=__( 'Import successfully done.', 'WPsmartcrm' );
            header("Content-type: application/json");
...
function WPsCRM_mail_to_customer(){
	if(check_ajax_referer('mailToCustomer', 'security',false ) && current_user_can('manage_crm') ){
		...
		$newMail = new testCRM_mail( array( 'customerID'=>(int)$_REQUEST['id_cliente'] ) );
		...
		if ( $_REQUEST['mailNow'] == 1 ){
			if( $_REQUEST['toCustomer']== 1 ){
				$newMail->sendMailToCustomer($mailfrom, $mailto, $_REQUEST['subject'], $_REQUEST['message'], date("Y-m-d H:i"), $_REQUEST['id_cliente'], $headers, $attachments);
			}
...
{
	$arr=array(__("Low","WPsmartcrm"), __("Normal","WPsmartcrm"),  __("High","WPsmartcrm" ) );
?>
...
?>
	...
	<head>
		...
		<title><?php _e('New User Registration on','WPsmartcrm')?> <?php bloginfo('name')?></title>
	
	</head>
	<body>
	<p><?php _e('A very special welcome to you','WPsmartcrm')?>, <b><?php echo $display_name ?></b>. <?php _e('Thank you for joining','WPsmartcrm')?> <i>"<?php bloginfo('name')?>"</i>!</p>
	
	<p>
		<?php _e('Your password is','WPsmartcrm')?> <strong style="color:orange"><?php echo $plaintext_pass ?></strong> <br>
    ...
    switch ($pass_strenght){
        ...
        case 'veryweak':  
            ...
            $_message=__('Your password strenght has been detected as','WPsmartcrm').'<strong>'.__('Very Weak','WPsmartcrm').'!!</strong>.'.__('We cannot accept users with weak passwords since the security of the whole site could be compromised','WPsmartcrm').'<br>'.__('Therefore you have to update your password using special chars, capital and normal letters and numbers ( min 8) click here to update it','WPsmartcrm').': <a href="'.get_page_link($user_page).'">'.__('Your Profile','WPsmartcrm').'</a>, '.__('thank you','WPsmartcrm');
            break;
        case 'weak':
            $_message=__('Your password strenght has been detected as','WPsmartcrm').'<strong>'.__('Weak','WPsmartcrm').'!!</strong>.'.__('We cannot accept users with weak passwords since the security of the whole site could be compromised','WPsmartcrm').'<br>'.__('Therefore you have to update your password using special chars, capital and normal letters and numbers ( min 8) click here to update it','WPsmartcrm').': <a href="'.get_page_link($user_page).'">'.__('Your Profile','WPsmartcrm').'</a>, '.__('thank you','WPsmartcrm');
            break;
        case 'medium':
            $_message=__('Your password strenght has been detected as','WPsmartcrm').'<strong>'.__('Medium','WPsmartcrm').'!!</strong>.'.__('We cannot accept users with weak passwords since the security of the whole site could be compromised','WPsmartcrm').'<br>'.__('Therefore you have to update your password using special chars, capital and normal letters and numbers ( min 8) click here to update it','WPsmartcrm').': <a href="'.get_page_link($user_page).'">'.__('Your Profile','WPsmartcrm').'</a>, '.__('thank you','WPsmartcrm');
            break;
        case 'strong':
            break;
    }
    ...
    ?>
        ...
        <p>
		<?php _e('Kind regards','WPsmartcrm')?>,<br>
			<?php bloginfo('name')?> 
        </p>
        </body>
        </html>
...
function WPsCRM_insert_notification($subscriptionrules, $document_id, $document_row_id, $scheduler_id)
{
    ...
    if ($document_id)
    {
        ...
        $scheduler_subject=__("Expiring service",'WPsmartcrm');
        $scheduler_notes= __("Expiring service",'WPsmartcrm');
        $scheduler_notes.= " ".$descrizione." ";
        $scheduler_notes.=__("Invoice #",'WPsmartcrm');
        $scheduler_notes.=" ".$prog." ";
        $scheduler_notes.=__("to",'WPsmartcrm');
...
function WPsCRM_save_crm_user_fields() {
    ...
    $wpdb->update(
			  $table,
			  array('nome' => $fields["nome"],'cognome' => $fields["cognome"],'indirizzo' => $fields["indirizzo"],'cap' => $fields["cap"], 'localita' => $fields["localita"], 'telefono1' => $fields["telefono1"], 'email' => $fields["email"], 'ragione_sociale' => $fields["ragione_sociale"], 'p_iva' => $fields["p_iva"], 'cod_fis' => $fields["cod_fis"]),
			array('ID_clienti'=>$_POST["client_ID"]),
...
function WPsCRM_view_activity_modal() {
	...
	if ($id_subscriptionrules=$riga->fk_subscriptionrules)
	{
		...
		$remindToCustomer=$steps[0]->remindToCustomer=="on"?__('Yes','WPsmartcrm'):__('No','WPsmartcrm');
    ...
    switch ($riga->tipo_agenda)
    {
        case 1:
            ...
            $icon='<i class="glyphicon glyphicon-tag"></i> '.__('TODO','WPsmartcrm');
            break;
        case 2:
            $icon='<i class="glyphicon glyphicon-pushpin"></i> '.__('Appointment','WPsmartcrm');
            break;
        case 3:
            $icon='<i class="glyphicon glyphicon-option-horizontal"></i> '.__('Activity','WPsmartcrm');
            break;
        default:
            $tipo="";
            break;
    ...
    <div class="panel-heading" style="    padding: 3px 10px;">
        <h3 style="text-align:center;margin-top: 8px;">'.__('Activity detail','WPsmartcrm').': <small style="margin-left:40px;color:#fff">'.$icon.'</small></h3>
    </div>
...
<table border="0" cellpadding="2" cellspacing="2" class="view table">
    <tr>
	    <td class="view">'.__('Customer','WPsmartcrm').':</td>
	    <td class="view_text"><a href="'.admin_url('admin.php?page=smart-crm&p=clienti/form.php& ID='.$riga->fk_clienti).'">'.$cliente.'</a></td>
    </tr>
    <tr>
	    <td class="view">'.__('Contact','WPsmartcrm').':</td>
	    <td class="view_text">'.$contatto.'</td>
    </tr>
    <tr>
	    <td class="view">'.__('Subject','WPsmartcrm').':</td>
	    <td colspan="2" class="view_text">'.stripslashes($riga->oggetto).'</td>
    </tr>
    <tr>
	    <td class="view">'.__('Description','WPsmartcrm').':</td>
	    <td colspan="2" class="view_text">'.stripslashes($riga->annotazioni).'</td>
    </tr>
    <tr>
	    <td class="view">'.__('Users','WPsmartcrm').':</td>
	    <td class="view_text">'.stripslashes($destinatari).'</td>
    </tr>
    <tr>
	    <td class="view">'.__('Groups','WPsmartcrm').':</td>
	    <td class="view_text">'.stripslashes($groups).'</td>
    </tr>
    <tr>
	    <td class="view">'.__('Days in advance','WPsmartcrm').':</td>
	    <td class="view_text">'.$days.'</td>
    </tr>';
    if ($riga->tipo_agenda==2)
    {
    $html.='<tr>
	    <td class="view">'.__('Send email to customer','WPsmartcrm').':</td>
	    <td class="view_text">'.$remindToCustomer.'</td>
    </tr>';
    };
    $html.='
    <tr>
	    <td class="view">'.__('Send mail to recipients','WPsmartcrm').':</td>
	    <td class="view_text">'.$mailToRecipients.'</td>
    </tr>
    <tr>
	    <td class="view">'.__('Publish on Account dashboard','WPsmartcrm').':</td>
	    <td class="view_text">'.$userDashboard.'</td>
    </tr>
    <tr>
	    <td class="view">'.__('Publish on group dashboard','WPsmartcrm').':</td>
    ...
    $html.='
    <tr>
	    <td class="view">'.__('Start','WPsmartcrm').': </td>
	    <td class="view_text"> '.WPsCRM_culture_date_format($riga->start_date).'</td>
    </tr>
    <tr>	
	    <td class="view">'.__('End','WPsmartcrm').':</td>
    ...
    else
    {
        $html.='
    <tr>
	    <td class="view">'.__('Expiration','WPsmartcrm').': </td>
	    <td class="view_text"> '.WPsCRM_culture_date_format($riga->start_date).'</td>
    </tr>';
    };
    $html.='
    <tr>	
	    <td class="view">'.__('Priority','WPsmartcrm').':</td>
	    <td class="view_text">'.$priorita.'</td>
    </tr>
</table>

<fieldset>
    <legend>'.__('Result','WPsmartcrm').'</legend>
    <table border="0" cellpadding="2" cellspacing="2" width="98%">
    <tr>
	    <td>'.__('To be done','WPsmartcrm').' <input type="radio" name="fatto" value="1" id="f_1" ';
	$riga->fatto=="1" ? $html.="checked" : $html.="";
	$html.='></td><td>';
	$html.=''.__('Done','WPsmartcrm').' <input type="radio" name="fatto" value="2" id="f_2" ';
	$riga->fatto=="2" ? $html.="checked" : $html.="";
	$html.='></td><td>';
	$html.=''.__('Canceled','WPsmartcrm').' <input type="radio" name="fatto" value="3" id="f_3" ';
	$riga->fatto=="3" ? $html.="checked" : $html.="";
	$html.='></td>
    </tr>
    <tr>
	    <td>'.__('Annotations','WPsmartcrm').'</td>
	    <td colspan="2" style="padding-top:20px"><textarea rows="3" cols="50" name="esito" id="esito">'.stripslashes($riga->esito).'</textarea></td>
    </tr>
    </table>
</fieldset>
          <span class="button button-primary _flat" id="save_activity_from_modal" data-id="'.$ID.'">'.__( 'Save', 'WPsmartcrm' ).'</span>
          <span class="button button-secondary _flat _reset" id="configreset">'.__( 'Reset', 'WPsmartcrm' ).'</span>
...
function WPsCRM_scheduler_update(){
	...
	if(check_ajax_referer('update_activity', 'security',false ) && current_user_can('manage_crm') ){
		$a_table=WPsCRM_TABLE."agenda";
		$id_agenda=(int)$_POST["ID"];
		$result=$wpdb->query( 
			$wpdb->prepare(
				"UPDATE $a_table SET fatto=%s, esito=%s WHERE id_agenda=%d",
				$_POST["fatto"], $_POST["esito"] ,$id_agenda
				) 
			);
		if($result > 0){
...
function WPsCRM_save_client_partial(){
	if(check_ajax_referer('update_document', 'security',false ) && current_user_can('manage_crm') ){
		...
		parse_str($_POST['values'], $arr);
		 $wpdb->update(
			  $table,
			  array('indirizzo' => $arr["indirizzo"],'cap' => $arr["cap"], 'localita' => $arr["localita"], 'provincia' => $arr["provincia"], 'p_iva' => $arr["p_iva"], 'cod_fis' => $arr["cod_fis"]),
			array('ID_clienti'=>$arr['hidden_fk_clienti']),
			  array('%s','%s','%s','%s','%s','%s')
		);
...
function WPsCRM_show_only_future_activity(){//TEMP da implementare per mostrare gli eventi passati nelle grid
	...
	$options['future_activities'] =="1" ? $return = true : $return=false;
	return $return;
}
...
function WPsCRM_check_client_one(){
	...
	if(  empty ( $clients ) ){
	    $wpdb->insert(
			  $table,
			  array('FK_aziende' => "$ID_azienda",'nome' => $options['business_name'],'cognome' => '','ragione_sociale' => $options['business_name'],'indirizzo' => $options['business_address'],'cap' => $options['business_zip'], 'localita' => $options['business_town'], 'provincia' => '', 'telefono1' => $options['business_phone'],'telefono2' => '','fax' => $options['business_fax'], 'email' => $options['business_email'], 'sitoweb' => '', 'skype' => '', 'p_iva' => $options['business_iva'], 'cod_fis' => $options['business_cf'], 'annotazioni' => '','data_inserimento'=>$data_inserimento,'provenienza'=>'','agente'=>''),
	    ...
	    $wpdb->update(
			  $table,
			  array('FK_aziende' => "$ID_azienda",'nome' => $options['business_name'],'cognome' => '','ragione_sociale' => $options['business_name'],'indirizzo' => $options['business_address'],'cap' => $options['business_zip'], 'localita' => $options['business_town'], 'provincia' => '', 'telefono1' => $options['business_phone'],'telefono2' => '','fax' => $options['business_fax'], 'email' => $options['business_email'], 'sitoweb' => '', 'skype' => '', 'p_iva' => $options['business_iva'], 'cod_fis' => $options['business_cf'], 'annotazioni' => '','data_modifica'=>$data_modifica,'provenienza'=>'','agente'=>''),
			array('ID_clienti'=>1),
			  array('%d','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%d')
		);
	}
...
function WPsCRM_get_countries($selected){
	ob_start();
    ?>
	<option value="0"><?php _e('Select...','WPsmartcrm') ?></option>
...
function WPsCRM_agent_can(){
	...
	$options = get_option( 'CRM_general_settings' );
	$user_can=false;
	if ( current_user_can('manage_options') || $options['crm_agent_can'] ==1 )
		$user_can= true;
	return $user_can;
}
...
function WPsCRM_generate_document_HTML($ID)
{
	...
	if ($ID)
	{
		...
		switch ($tipo=$riga["tipo"])
		{
			case 1:
				...
				$document_name=__("Quote",'WPsmartcrm');
			...
			case 2:
				$progressivo=$riga["progressivo"];
				$document_name=  __("Invoice",'WPsmartcrm');
			...
			case 3:
				$progressivo=$riga["id"];
				$document_name=  __("Informal invoice",'WPsmartcrm');
				$text_before=$document_messages['invoices_before'];
				$text_after=$document_messages['invoices_after'];
				$document_prefix=$document_numbering['invoices_prefix'];
				$document_suffix=$document_numbering['invoices_suffix'];
	...
	$subheader  ='';
	$subheader .=
		'<table class="WPsCRM_customer_data" cellspacing="0" style="width: 100%; text-align: left; font-size: 11pt;">
			<tr>
				<td style="width:60%;vertical-align:top"><b>'.$document_name.' # '.$n_offerta.' '.__("issued on",'WPsmartcrm').' '.WPsCRM_culture_date_format($riga["data"]).'</b></td>
				<td style="width:40%; ">'.$document_dear.' <b>'.$cliente.'</b><br />
				'.$indirizzo.'<br />'.$cap.'  '.$localita.'     ( '.$provincia.' )';
				if ($p_iva)
					$subheader.='<br />'.__('VAT code','WPsmartcrm').': '.$p_iva;
				if ($cod_fis)
					$subheader.='<br />'.__('Fiscal code','WPsmartcrm').': '.$cod_fis;
				if ($contatto)
					$subheader.='<br />C.A. '.$contatto;
				if ($riferimento)
					$subheader.='<br />'.__('Reference #','WPsmartcrm').': '.$riferimento;
	$subheader.='</td>
			</tr>
		</table>';
	//$subheader .='<div class="col-md-6 WPsCRM_document-data"><b>'.$document_name.' # '.$n_offerta.' '.__("issued on",'WPsmartcrm').' '.WPsCRM_culture_date_format($riga["data"]).'</b></div>';
	//$subheader .='</section>';
	//document body
	if ($oggetto)
		$doc_body.='<h4 class="WPsCRM_document_subject">'.__('Subject','WPsmartcrm').': '.$oggetto.'</h4>';
	if ($text_before)
	...
	$tab_cond="";
	if ($pagamento)
		$tab_cond.="<tr style=\"background:transparent!important\"><td style=\"background:transparent!important\">".__("Payment",'WPsmartcrm').":</td><td>".$pagamento."</td></tr>";
	if ($riga["annotazioni"])
		$tab_cond.="<tr style=\"background:transparent!important\"><td colspan='1' style=\"background:transparent!important\">".__("Notes",'WPsmartcrm').":</td><td colspan='1' style='font-size:.9em;font-style:italic'>".stripslashes($riga["annotazioni"])."</td></tr>
	";
	$doc_body.=$t_articoli;
	//if ($tab_tot && $tipo==2)
	//if ($tab_tot)
		...
		$doc_body.='<div class="col-md-5 pull-left" style="padding:0">
			...
			<thead>
					<th style="border:none">
						<h4>'.__("Conditions",'WPsmartcrm').':</h4>
					</th>
				</thead>
...
function WPsCRM_create_pdf_document($ID, $content, $attachment=0, $old_file, $tipo, $WOOmail)
{
	...
	if ($attachment==1)
	{
		...
		$subject= sprintf( __( 'Invoice issued on %1$s from %2$s', 'WPsmartcrm' ), date("d-m-Y"), $from_name);
		//$message = "Gentile cliente, ti ringraziamo per il tuo acquisto. In allegato trovi la tua fattura.". PHP_EOL;
		$message= __( 'Dear customer, thank you for your purchase. Your invoice is attached.' , 'WPsmartcrm' );;
		$message.="\r\n";
		$message.=__( 'Kind regards', 'WPsmartcrm' );
		$message.=PHP_EOL;
		//$message .= "Clicca il seguente link per accedere al tuo pannello di controllo:  <a href=\"".get_page_link($user_page)."\">".__("Profile page.",'WPsmartcrm')."</a>";
...
<div class="col-md-6">
	<label style="font-weight:normal;margin-top:10px;padding:8px 0">
		<?php _e("Print 'PAID' if paid","WPsmartcrm")?>
	</label>
...
<div class="col-md-6">
	<label style="font-weight:normal;margin-top:10px;padding:8px 0">
		<?php _e("Print 'Non formal' ","WPsmartcrm")?>
	</label>
	<input type="checkbox" name="printPaid" id="printPaid" value="1" checked />
</div>
<?php } ?>
	...
	if ( in_array( 'newsletter/plugin.php', apply_filters( 'active_plugins', $filter) ) ) 
	{
		...
		if ($email)
		{
			...
			if ($riga)
			{
				...
				{
					case "C":
						$msg= __('A record with this email already exixts in state "Confirmed" ','WPsmartcrm');
						break;
					case "S":
						$msg=__('A record with this email already exixts in state "Not Confirmed" ','WPsmartcrm');
						break;
					case "U":
						$msg=__('A record with this email already exixts in state "Unsubsrcibed" ','WPsmartcrm');
						break;
					case "B":
						$msg=__('A record with this email already exixts in state "Bounced" ','WPsmartcrm');
						break;
					default:
						$msg=__('A record with this email already exixts ','WPsmartcrm');
						break;

				}
				?>
				<div class="col-md-6 pull-right _customer_newsletter">
                    <h3><?php _e('Newsletter management','WPsmartcrm')?></h3>
					<div class="row form-group">
						<label class="control-label"><?php echo $msg ?> <small><a href="<?php echo admin_url('admin.php?page=newsletter_users_index')?>" target="_blank"><?php _e('Manage subscribers','WPsmartcrm')?>&raquo;</a></small></label>
					</div>
					<input type="hidden" name="newsletter_flag" id="newsletter_flag" value="1">
				<?php
					$options_profile = get_option('newsletter_profile');
					echo '<div class="newsletter-preferences-group row form-group"><h6 style="border-top:1px solid #ccc;padding-top:6px">'.__('Add record to specific newsletter lists','WPsmartcrm').':</h6>';
			...
			else
			{
				...
				<div class="row form-group">
					<label class="col-sm-4 control-label"><?php _e('Insert in Newsletter','WPsmartcrm')?>?</label>
					<div class="col-sm-1">
